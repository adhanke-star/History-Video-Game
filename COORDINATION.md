...
### LANE-003 · battle-ladder — **DRIVE** (Claude Code / Claude Fable; D382 mega-run ladder — current slice: Fort Donelson SPEC ONLY)
...
- **Resume pointer:** **D381 is the current clean release boundary; the next slice is Fort Donelson SPEC ONLY** from `docs/design/battle-build-research/naval-river-battle-build-research.md` (READY_FOR_SPEC): take DRIVE in a committed ledger edit, author `docs/design/fort-donelson-battle-build-spec.md` plus a bind-tested dual-mode plan probe on the D379 pattern (land+river gunboat-support inputs, no ship-vs-ship engine, exact battle-date grades incl. the Floyd/Pillow/Buckner command devolution and the "unconditional surrender" teaching beat), commit that green planning boundary, then build runtime in a separate bounded slice. Aaron's 2026-07-12 popup Q&A (D382) governs the wider forward slate. Three transfer-era plan-probe reds were root-caused and greened in the D381 commit itself: the five-forks/gaines LANE teeth were reshaped from current-lock-holder pins onto durable roster/history anchors (the ledger's own documented pattern), and the loot probe's dropped `D362: 912 -> 957` pin-history fragment was restored; binds proved the reshaped teeth bite exactly. **Historical pointer:** D362 playable Gaines' Mill is the handoff boundary in the commit that
...
- **History:** lane opened by Claude Code 2026-07-10 after Aaron's popup Q&A (D359) fixed scope,
  ...
  non-Leetown spec per the D382 order).
