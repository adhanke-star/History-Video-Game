...
### LANE-003 · battle-ladder — **DRIVE** (Claude Code / Claude Fable; D396 Petersburg initial-assaults PLANNING contract only)

- **Owning tool:** Claude Code / Claude Fable (Fable 5, xhigh), DRIVE for the D396 Petersburg initial-assaults PLANNING contract ONLY — the D395-named June 15-18, 1864 initial assaults (the Dimmock Line; Beauregard's initial defense): research-verify pass → `docs/design/petersburg-initial-assaults-battle-build-spec.md` + a dual-mode, filesystem-first, fail-closed, bind-tested plan probe; **RUNTIME EXCLUDED** (no `data/petersburg*.json`, no T1/registry/menu/schema/coverage/suite/sweep/Army-Register movement, no simulation input). The take is this ledger-only commit at the clean pushed D395 boundary `d099082` (HEAD == origin/main verified; all eleven plan probes ALL OK at the boundary). Before this take: none. D394 discharged ChatGPT/Codex's D393 VERIFY ownership after the complete serialized release battery, artifact audit, narrow Five Forks stale-baseline fix, docs sync, and push. The standing serialization law survives: no provider takes DRIVE without a committed ledger edit, and release batteries run alone.
- **State:** DRIVE (planning only). D393's playable Wilderness remains the runtime boundary at **23 scenarios / schema 53 / Army Register 1434 / coverage 23 / suite 128 / sweep 23**; D394 verified all 128 commands, 127/127 expected JSONs, schema 53/53, and sweep 23×8=184 with failures 0. No runtime/data/generated-game/simulation input moved in D394 or in the D395 docs-only decision checkpoint. **D395 (Aaron, 2026-07-14) resolved the D394 decision-only boundary: D382 item 3.5's Petersburg trench treatment is NAMED — the initial assaults of June 15-18, 1864 (the Dimmock Line; Beauregard's initial defense), entering a later spec-first planning lane with runtime excluded (the recommended explicit waiver declined); Cold Harbor is DEFERRED (packet-High does not authorize silently expanding the ratified order; READY_FOR_SPEC, insertable only by an explicit Aaron reorder). The authoritative next bounded task is the Petersburg initial-assaults planning contract under a fresh committed ledger-only DRIVE take; the war-career loop (D382 item 4) follows the completed 3.5 rung.**
...
- **Acceptance criteria (per battle, the full D330-D335 pattern + extras per Aaron's popup):**
  ...
  5. Roster/custom-builder baselines updated; **the current probe-loot-survival registry pin is 1434**
     (+units×3, documented-history comment retaining D393 `1380 -> 1434` and earlier transitions — see LANE-002 note); teaching cards + codex axes;
