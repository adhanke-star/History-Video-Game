...
### LANE-003 · battle-ladder — **DRIVE** (Claude Code / Fable — Spotsylvania planning contract ONLY)

- **Owning tool:** Claude Code / Claude Fable (DRIVE take 2026-07-13, ledger-only commit at clean D389 boundary `3ba2c93`), scoped to the planning-only standalone Spotsylvania / Bloody Angle contract and nothing else; ChatGPT/Codex completed D389 and released VERIFY at the clean 126/126 boundary. The standing serialization law survives every transfer: release batteries run alone on this machine — no simultaneous edits by any provider, one Chrome probe at a time.
- **State:** DRIVE — Spotsylvania planning contract only. **Acceptance contract:** (1) re-verify the attrition packet's §9 spec-time gaps (division-grain May 12 OOB on the Mule Shoe/Bloody Angle objective, Anderson's dated command title/grade for May 8-21, Grant's Lt.-Gen. commission source URLs, Sedgwick-death/Wright-transfer, Longstreet absence, IX Corps independence, a phase-grain casualty-direction claim, weather/fog treatment, terrain anchors, and the D90 defender-hold lean) and record the adjudication as a packet §12 addendum; (2) write `docs/design/spotsylvania-battle-build-spec.md` (standalone single phase, May 12 1864, US Hancock II + Wright VI attacker vs CS Ewell Second Corps defender, future menu rank 68, D90 defender-hold lean, D74 wall, direction guard 8 seeds ≥5/8 direction-only); (3) write `tools/probe-spotsylvania-plan.mjs` — filesystem-first, dual-mode, fail-closed, twelve named steps, exact one-token Anderson-rank bind; (4) run the focused planning gate (build GATE OK byte-identical, schema 51/51, research 15/15, all ten plan probes green, suite 126, all D389 hashes/baselines held); (5) docs sync + release to CONTRACT/unowned; stop before runtime/data/registry/count movement. D388's playable commit `67f9672a6ff8c734c7f0ec6fa385fb7d5ad700e8` is release-verified. Elkhorn Tavern remains scenario 21 at scenarios/schema/register/coverage/suite **21/51/1326/21/126**, menu rank 49, sweep 21, T10 `TM / false / first-national`, and four direction locks 8/8 with zero tuning. D389 completed all 126 manifest commands in four exact-label segments (77+14+11+24), audited all 125 expected JSONs, schema 51/51, sweep 21×8=168, and all 153 fresh image artifacts. Runtime/data/simulation inputs did not move. Three release-only fixes bounded Arms capture/cleanup, strengthened Tripo's no-duplicate-base tooth, and corrected NMH's complete chronology tooth. **No D382 feature work has started.** The next owner takes DRIVE in a committed ledger-only edit for the planning-only standalone Spotsylvania / Bloody Angle contract from the READY_FOR_SPEC attrition packet, re-verifies its spec-time gaps, ships the spec + dual-mode bind-tested plan probe, and stops before runtime.
...
- **Resume pointer (authoritative, D390 DRIVE):** the lock is taken; **start with the attrition packet's §9 verification gaps** (`docs/design/battle-build-research/1864-65-attrition-battle-build-research.md` §9/§11): re-verify division-grain May 12 OOB, Anderson dated command/grade, Grant rank-source URLs, and casualty-direction support via read-only gather → default-refute helpers with Fable adjudication; then the spec, the twelve-step plan probe, the Anderson-rank bind, the planning gate, docs sync, commit/push, release to CONTRACT/unowned, and **stop before runtime/data/registry/count movement**. HALT-and-release if division-grain OOB, Anderson's dated status, Grant's source URLs, or a defensible phase-direction rule cannot be resolved. *(Prior D389 pointer, discharged by this take: no D382 work had started; the planning-only Spotsylvania slice was the exact next action.)*
...
- **Last-touched commit:** D362 (Gaines' Mill playable slice + Fable transfer) · D363 (New
  ...
  Arms/Tripo/NMH safety-net fixes, clean CONTRACT release, ChatGPT/Codex 5.6 Sol Ultra, this
  commit).
- **History:** lane opened by Claude Code 2026-07-10 after Aaron's popup Q&A (D359) fixed scope,
  ...
  Spotsylvania / Bloody Angle as the next pointer.
