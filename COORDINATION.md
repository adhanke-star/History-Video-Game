...
### LANE-003 · battle-ladder — **DRIVE** (Claude Code / Claude Fable; D382 mega-run ladder — D383 spec SHIPPED; current slice: D384 Fort Donelson runtime)

- **Owning tool:** Claude Code / Claude Fable — took DRIVE 2026-07-12 at clean `257bea8` for the Aaron-chartered D382 mega run (Fort Donelson spec → runtime → Women-in-War arc → Elkhorn spec, each its own bounded commit). The standing serialization law survives every transfer: release batteries run alone on this machine — no simultaneous edits by any provider, one Chrome probe at a time.
- **State:** DRIVE — **D381 discharged the 124-command release checkpoint** owed since D380: the full serialized `npm run vet:noreg` battery is green across evidenced segments, every fresh artifact was read (123/123 suite JSONs clean; sweep 19 scenarios × 8 seeds with `failures:[]`; schema HTML exactly 49 rows; suite manifest 124; Five Forks pins 19/49/1200/19/124 all held; generated HTML `10a64a20394521efdc94b7edb1646686`). D380 playable Five Forks remains pushed at `cd1a08f871e57fa27f1f5d03cc8ceee15a1d8e57`; the D381 evidence and the two root-caused non-regressions (the tactical-visuals slow-Mac timeout, the frozen-engine `ford_albedo` optional-asset 404) are recorded in DECISIONS D381. Durable ladder history: D375 (2026-07-11) locked the Cedar Creek contract; D376 shipped it playable; D377/D378 shipped Cross Keys / Port Republic; D379/D380 shipped Five Forks; D381 verified the release.
...
- **Resume pointer:** **D383 is the current committed planning boundary; the next slice is D384 PLAYABLE FORT DONELSON RUNTIME** from the committed `docs/design/fort-donelson-battle-build-spec.md` (md5 `4a8c4e226e4224e1486383f806782118`; built from `docs/design/battle-build-research/naval-river-battle-build-research.md` READY_FOR_SPEC plus its D383 §12 addendum). The D384 contract in the spec's §11/§14 controls: `data/fort-donelson.json` three-phase T8 (roles US>CS / CS>US / US>CS, weights [1,1,3], fog off, the Feb 14 naval-repulse interstitial on Phase 2's transition card, the recall teaching on Phase 3's), committed strengths inside the §3 envelopes under the corrected US 24,531 / CS 16,171 engaged anchors, T1 registry + `fortDonelson:48` before Shiloh, schema 50, roster/builder + `PHASE_COUNTS fortDonelson: 3`, Army Register `1200 + units×3` with the FULL documented pin chain, T10 `W / false / first-national`, coverage 20, suite 125 + sweep comment 20, the focused probe's 8-seed four-guard battery (P1 CS holds · P2 CS seizes · P3 US seizes · aggregate US, ≥5/8 each, NO casualty-direction tooth), the early-1862 rank wall (Grant Brig. Gen.; Buckner never Lt. Gen.; Forrest Lt. Col.; Foote Flag Officer), gunboats as inputs only, both binds (registry removal + Grant-rank tamper) with md5 restores, honest A/B if any input moves, docs sync, commit "D384: ship playable Fort Donelson," push. Aaron's 2026-07-12 popup Q&A (D382) governs the wider forward slate. Three transfer-era plan-probe reds were root-caused and greened in the D381 commit itself: the five-forks/gaines LANE teeth were reshaped from current-lock-holder pins onto durable roster/history anchors (the ledger's own documented pattern), and the loot probe's dropped `D362: 912 -> 957` pin-history fragment was restored; binds proved the reshaped teeth bite exactly. **Historical pointer:** D362 playable Gaines' Mill is the handoff boundary in the commit that
...
- **History:** lane opened by Claude Code 2026-07-10 after Aaron's popup Q&A (D359) fixed scope,
  ...
  Claude Fable, DRIVE retained for D384).
