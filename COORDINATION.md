...
### LANE-003 · battle-ladder — **VERIFY** (D380 playable Five Forks; release battery resumes at command 38)

- **Owning tool:** Claude Code / Claude Fable as TOP LOOP. ChatGPT/Codex stopped on Aaron's 2%-usage request and transferred VERIFY here. No simultaneous Codex edits; no simultaneous edits by any provider.
- **State:** VERIFY — D380 playable Five Forks is pushed at `cd1a08f871e57fa27f1f5d03cc8ceee15a1d8e57`. Playable Five Forks, the generic shared T3 relief seam, both runtime binds, atomic 19/49/1200/19/124 integration, and the complete serialized focused/adjacent gate are green. Commands 1-37 of the 124-command release battery have evidence; command 38 (`women in war`) was interrupted only for the requested provider transfer and is the exact resume boundary.
...
- **Current objective:** from a clean fetched boundary, confirm no browser/helper/research process is active, keep the shared Python server or start one on port 8765, and run `export TMPDIR="$PWD/.tmp" && node tools/vet-no-regression.mjs --from='women in war'` alone. Complete commands 38-124 without concurrent work, read every fresh JSON and screenshot/canvas artifact across all evidence segments, require the 19-scenario × 8-seed sweep with `failures:[]`, record D381, and release the lane to CONTRACT/unowned. Do not start Fort Donelson inside VERIFY.
- **D380 and release-battery resume pointer:** focused hashes remain T3 `7c3b0924c94e6f450e2fd491726a022f`, T1 `85c12c00440499a7bddc67060e9913fc`, data `380150cee52d99f7e10cbe7b45321f1a`, generic probe `da4934bbfc3bdeecca1150aff66045ed`, focused probe `e4da87df501b2ba50bdab4cf6857df27`, and generated HTML `10a64a20394521efdc94b7edb1646686`. First-authored direction is 8/8 US seizure and 8/8 CS losses greater with zero simulation-input changes; both binds matched only their declared teeth. Release evidence is segmented: commands 1-5 in `.tmp/vet-no-regression-2026-07-12T12-21-32-130Z.log`; boot command 6 standalone green; commands 7-19 in `.tmp/vet-no-regression-2026-07-12T12-38-07-503Z.log`; commands 20 and 21 standalone green after deterministic post-result exit fixes in `probe-irregular-war.mjs` and `probe-under-told-perspectives.mjs`; commands 22-31 in `.tmp/vet-no-regression-2026-07-12T13-25-32-052Z.log`; commands 32-37 in `.tmp/vet-no-regression-2026-07-12T14-04-51-793Z.log`. The last segment was stopped by Ctrl-C during command 38, so rerun command 38 rather than trusting any partial artifact. An earlier false dirty tree/partial JS read/missing runtime symbol was traced to macOS `compressed,dataless` placeholders: every tracked file was explicitly hydrated and zero tracked files remain dataless; 9,310 ignored/untracked stale artifacts remain dataless. If a partial read recurs, materialize the named path with `brctl download <path>` and verify its blob before acting. The two exit fixes change no assertion, screenshot requirement, or red/green criterion; each standalone probe is green at 8/8 with zero page errors.
...
- **Resume pointer:** **D380 playable and focused-gated is the current handoff boundary; commands 1-37 of the release battery are evidenced green.** Fetch, require the transfer commit and clean parity, then resume the serialized battery exactly at `women in war` using the command above. Do not rerun the proven prefix unless an artifact freshness audit identifies a specific gap.
