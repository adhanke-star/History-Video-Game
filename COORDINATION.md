...
- **Resume pointer (authoritative, D389 CONTRACT):** no D382 work has started. Take DRIVE in a committed ledger-only edit, then perform the planning-only slice in `docs/design/battle-build-research/1864-65-attrition-battle-build-research.md` §11: re-verify §9's division-grain May 12 OOB, Anderson dated command/grade, Grant rank-source URLs, and casualty-direction support; write a standalone Spotsylvania / Bloody Angle spec plus a filesystem-first dual-mode bind-tested plan probe; run the planning gate, commit/push, release to CONTRACT/unowned, and **stop before runtime/data/registry/count movement**.
