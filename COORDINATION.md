...
- **State:** CONTRACT. D532 terminal result is `2 ROAD_GAP_CURED`, `4 ROAD_GAP_UNRESOLVED`, and
  ...
  campaign start, load, save acceptance, UI, migration, topology, control, service condition, army,
  date, order, movement, or operational default exists. D511 completed the fourth road-evidence pass and retained exact
...
- **D527 historical DRIVE take:** from clean pushed D526
  `24c7a2b051fa1f4fafefe17ba5f3908b2081a221`, ChatGPT/Codex owned only the exact
  docs/research rail-window adjudication contract below.
...
- **D528 delivery record — rail physical-window research:** the existing D497 packet now contains
  one normative exact 27+4 audit. The result is `0 PHYSICAL_WINDOW_ESTABLISHED`,
  `19 PHYSICAL_PRESENCE_SNAPSHOTS_ONLY`, `7 PHYSICAL_EXISTENCE_UNRESOLVED`, and
  `1 PHYSICAL_EXISTENCE_DISPUTED`. The unresolved rows are `CTS-R-02`, `CTS-R-03`, `CTS-R-04`,
  `CTS-R-10`, `CTS-R-11`, `CTS-R-25`, and `CTS-R-26`; `CTS-R-27` alone remains disputed. No row
  has qualifying continuity evidence. The Galveston troop crossing remains nonphysical military-use
  context and cannot promote `CTS-R-26`; the Texas & New Orleans removal/inspection conflict retains
  precedence and supplies no settled end or Louisiana through rail.
- **D528 source and CTI record:** genuinely used, manually read sources append contiguously as
  `SR-27`..`SR-33`. Each claim resolves to one canonical underlying-work family/source/type and its
  exact role/locator/bounded precision. Military use, control, operation, supply, abandonment, and
  service cannot supply a physical bound. The admitted faces remain `CTI-01` / `RE-06`, `CTI-02` /
  `RE-22`, `CTI-03` / `RE-11`, and `CTI-04` / `RE-11`+`RE-12`; all four rail faces remain `INTERCHANGE_WINDOW_UNADJUDICATED`,
  with only `RAIL_FACE_EVIDENCE_ONLY` recorded.
- **D528 evidence and protected pins:** original `SR-01`..`SR-26`, node, candidate-edge, and
  case-study sections remain exact at 9,505 / 9,188 / 11,323 / 3,360 bytes and SHA-256
  `19650bb16dd653eb715107f5a1f1c5b53cea704ae1da468febeaa3fbb189577c` /
  `9287f673221fd6aef8ac58104b40f25b750a5b1b1c4dcaa89f61acf47f02ae08` /
  ...
  New `SR-27`..`SR-33` rows are pinned at 3,374 bytes / SHA-256
  `792b26c96f21b06a0078cf21d55d457f1a2c202326dd4a736067834cdd0f41f4`; exact ordered
  claim-bearing cells excluding disposition are 19,788 bytes / SHA-256
  `8f017adbd621a818c74001f82db2f70d22387daa1a47b452ce49af1307befdbc`.
  Transport JSON remains SHA-256 `7e484de1c1c59abc698b4cdfe3e599dd8e8c97207e611542eb965d54c38c90f6`;
  all 27 mappings and qualitative `dateText` values remain exact. D526 is archived byte-verbatim at
  4,723 bytes / SHA-256 `c156b0bbfbbddd6448a36a9fcedff3621def8819f13e4d18886a7a0a3391c044`.
- **D528 research proof:** dedicated proof 10/10; Bind R changes only `CTS-R-01` from
  `PHYSICAL_PRESENCE_SNAPSHOTS_ONLY` to `PHYSICAL_WINDOW_ESTABLISHED`, exits 1 at exactly 9/10 with
  only `DISPOSITION / POSITIVE FLOOR / CONTINUITY` red, then restores packet/probe bytes exactly.
  Both probe syntax checks, terminal transport plan 12/12, doc coherence 5/5, conquest plan 8/8,
  Mayhem plan 13/13, War Career plan 24/24, normal build/check `GATE OK`, exact 13-file allowlist,
  protected no-diff, fresh JSON, source readback, archive equality, and `git diff --check` pass.
  Packet/probe restore to SHA-256 `434abe606010d73a72e033dc9ead771684964dc9158c41963ef5efe6dfe46ed0` /
  `38784abaeba4bc154ab19e29ef6f3b34395d68a2c8b298d3e38317dc6dddf716`.
  Final independent Sol-high re-audit reports PASS after both evidence findings were fixed. No
  product/generated diff exists. No browser or full-suite gate is owed.
- **D527 historical resume pointer:** after D527 is committed, pushed, fetched, and proven clean, execute only this
...
- **Resume pointer:** D528 releases LANE-019 at `CONTRACT` / `none` with D525 still the product head.
  Under D514, reload the live completion ledger and separately adjudicate/contract the next
  dependency-ready prerequisite. Roads remain unavailable; no Historical rail legal-now window or
  full interchange exists; LANE-020 remains independently blocked. Do not infer data/runtime/current-
  turn/state/receipt/topology/route/movement/live-save authority from this docs-only finding.
- **D529 DRIVE take and dependency verdict:** from clean pushed D528
  `4ab8440859613ca1ef88db0896fda95bdfdb9a04`, ChatGPT/Codex owns only the exact D530 docs/research
  contract below. A Mayhem-only playable slice is not dependency-ready: D521's evidence view has no
  control, service condition, receipts, topology, order, opening values, or live/save lifecycle.
  The next smallest independent prerequisite is the remaining mode-bounded Historical source audit,
  not a new product owner.
- **D529 exact water/sea scope and sole artifact:** D530 works only in existing D499
  `docs/design/strategic-water-transport-research-packet.md`. Audit exact ordered
  `CTS-W-01`..`CTS-W-15` mapped to `WE-01`, `WE-03`, `WE-04`, `WE-05`, `WE-06`, `WE-07`, `WE-08`,
  `WE-09`, `WE-10`, `WE-11`, `WE-12`, `WE-14`, `WE-17`, `WE-21`, `WE-22`, followed by
  `CTS-S-01` / `WE-24` and `CTS-S-02` / `WE-26`. Append exactly one normative audit section/table.
  Existing `SW-01`..`SW-32` remain byte-identical and ordered; genuinely accessed/used new sources
  append contiguously from `SW-33`. Existing node, edge, chokepoint, transfer-case, and JSON
  qualitative `dateText` surfaces remain byte-identical. D499's old `READY_FOR_CONQUEST_LAW`
  remains topology/category-input readiness only. Create no packet, table owner, registry, schema,
  parser, data field, or runtime owner.
- **D529 physical-corridor claim law:** physical corridor/landing existence is distinct from generic
  navigability. Each row retains exact IDs, corridor, endpoints, direction, and mode, with separate
  presence, end/continued-existence, continuity, and nonphysical-context claims. Every claim carries
  one canonical underlying-work `familyKey`, resolved `SW-*`, role, type, precise locator, and
  verbatim/bounded precision. Different hosts of one work count once. Every counted source/locator,
  including snapshot, unresolved, and disputed evidence, must be accessed and manually read back.
  Direct physical evidence is limited to hydrographic/channel surveys, harbor/river engineering,
  landing/wharf property or inspection, or explicit official accounts of channel/cutoff/canal/
  landing/approach/obstruction/clearance/construction/survival/removal/destruction. Maps,
  photographs, passages, landings, transfers, approaches, convoys, and fleet movements prove at most
  dated snapshots. Military use, control, capture, port closure, blockade, service, lift, operation,
  water level, weather, draft, vessel class, and silence about hazards prove no standing interval or
  continuity.
- **D529 dispositions and positive floor:** deterministic precedence is explicit conflict →
  ...
  for the exact corridor and each asserted bound, one direct physical family supporting presence and
  continuity, and actual evidence across the whole asserted interval. Two snapshots do not establish
  continuity. Exact precision remains exact; no season or campaign boundary is invented. Zero
  positives is valid. All labels are docs-only and supply no JSON/runtime enum, legal-now result,
  navigability, control/condition, capacity/lift, adjacency, topology, route, or movement authority.
- **D529 permanent negatives and CTI exclusion:** Island No. 10's two operation paths remain
  distinct; Mississippi approach/control/passage/surrender/opening language is not physical
  continuity; Arkansas/Red low water creates neither permanent absence nor a season calendar; City
  Point clearance creates no standing service; Roanoke/New Bern creates no complete sounds graph;
  `WE-24` remains one-way/January-1865 bounded; and `WE-26` “closed thereafter” remains a
  blockade-runner closure rather than physical disappearance or generic transport closure. A
  contradiction with any shipped endpoint, edge, non-link, direction, or qualitative date HALTs.
  `CTI-01`..`CTI-04` are excluded and remain `INTERCHANGE_WINDOW_UNADJUDICATED`; D528's rail faces,
  water passages, landings, co-location, or service/control dates establish no water face or handling.
- **D529 probe and Bind W:** D530 may add suite-excluded
  `tools/probe-conquest-water-window-research.mjs` and update only the current-boundary tooth in the
  transport plan. The guard proves singular audit, preserved source/register surfaces, exact 15+2
  mapping/order/endpoints/direction, claim/source/family/type/locator resolution, precision/role
  separation, the direct/continuity floor, named negatives, CTI exclusion, and no product authority.
  Every appended source row must resolve from at least one audit claim and its recorded manual
  readback. Bind W removes enough qualifying families or claims from the first established row to
  deterministically violate the complete floor—leaving fewer than two independent families or
  removing required direct continuity support—or, at zero positives, promotes the first ordered
  non-disputed deficient row without curing it. Only
  `DISPOSITION / POSITIVE FLOOR / CONTINUITY` may red; packet/probe bytes restore exactly.
- **D529/D530 allowlists and gates:** D529 changes only `AUTONOMOUS-RUN.md`, `COORDINATION.md`,
  `DECISIONS.md`, `HANDOFF.md`, `RUN-LOG.md`, `START-HERE.md`, `V1-CHECKLIST.md`, `WAKE-UP.md`,
  Package A §8, `legacy/HANDOFF-ARCHIVE.md`, and the transport-plan current-boundary tooth. D530 may
  additionally change only the existing D499 packet and add the dedicated guard. Data, every `src/`
  and runtime probe, manifest/build/suite/save owner, rail/road packet, generated HTML, frozen base,
  and all unlisted files are barred. D529 proof is transport-plan syntax/12-of-12, doc coherence,
  conquest/Mayhem/War-Career plans, normal build/check, exact 9,097-byte D527 archive at SHA-256
  `93afa994eb7f381855de7edb1f504350be197143fff18bc6cefe9ee1f6915017`, exact eleven-file
  allowlist, protected/artifact/diff audits. D530 adds both-probe syntax, dedicated proof, all source
  readbacks, Bind W/restore, terminal proof, and exact 5,925-byte D528 archive at SHA-256
  `c4c96e7ef7e468532040e34bdec1ab6432aec4cb5b9a958a2ca7ec0b7ce941a4`, then repeats static gates.
  No browser/full battery is owed unless a protected/shared product surface moves.
- **D529 routing proof:** transport-plan syntax and 12/12; doc coherence 5/5; conquest plan 8/8;
  Mayhem plan 13/13; War Career plan 24/24; normal build/check `GATE OK` at data 65, territories 36,
  transport 27/15/2/4/18, manifest 112 with 116 last, and suite 140; exact 9,097-byte archive; exact
  eleven-file allowlist; protected no-diff; fresh artifacts; and `git diff --check`. Generated
  product, frozen base, transport JSON, and D499 packet hashes remain exact. No browser/full-suite
  gate is owed. Independent Sol-high audit found and closed deterministic-bind,
  appended-source-resolution, and under-toothed-contract gaps; fresh re-audit reports PASS with no
  remaining contradiction or overclaim.
- **D529 resume pointer:** after this routing contract is committed, pushed, fetched, and clean,
  execute only D530's 17-row research pass. Record the honest result and quarantines, prove all
  source/probe/bind/restore/gate requirements, commit/push/fetch clean, release LANE-019 to
...
- **D538 exact Slice-1 seam:** `_lgRoute(C, bd)` in `src/61-logistics-rail.js` keeps its shipped six-key
  return (`id`/`label`/`theater`/`theaterName`/`friction`/`note`) computed exactly as today and gains ONE
  guarded tail: when the carrier declares an exact conquest campaign kind AND the open ruleset, it attaches
  one extra derived `trace` field carrying a real path across the read-only 36-territory board. Nothing
  downstream is touched — `logisticsSnapshot`, `logisticsBridgeBonus`, `logisticsSetPriority`,
  `logisticsOnResolve`, `presLogisticsBlock` and `logisticsWireOverview` keep their current text, shape and
  behavior, so no snapshot field, no capped-bridge field and no DOM byte moves in ANY campaign, conquest
  included. No new module: the manifest stays at 112 with `116-conquest-state.js` last.
- **D538 exact Slice-1 containment seam (the declared bind target):** one ruleset gate returns a view ONLY
  for the open ruleset and returns `null` for every other value — including the sourced ruleset — BEFORE any
  board or transport-evidence read occurs. The boundary therefore fails CLOSED structurally at the ruleset
  seam rather than by label or filter, exactly as the containment contract above requires. The gate is the
  one value the D538 negative bind mutates.
- **D538 Slice-1 authored Mayhem content (openly authored game content, never a Historical claim):** two
  source depots (`US` → `CT-01`, `CS` → `CT-05`), four theatre fronts (`E` → `CT-03`, `W` → `CT-20`,
  `TM` → `CT-32`, `N` → `CT-19`), four per-mode segment costs, and the projection of each evidence row's
  existing `territoryRefs` into a directed graph that honours that row's existing `direction`. None of it is
  written into `src/115-conquest-transport.js`, none of it shares that module's namespace, and none of it is
  reachable on the sourced ruleset. New Orleans-origin, `CT-36`, the D503 endpoint quarantine, the
  Potomac/operation-composition/Sherman-chain negatives and the permanent unassignment of Boonville, Arrow
  Rock and Glasgow are untouched; `CTI-01`..`CTI-04` stay `INTERCHANGE_WINDOW_UNADJUDICATED` for Historical
  and are not consumed by the trace at all; roads remain absent with `ROAD_REQUIRES_BOUNDED_SOURCE_PASS`
  exact, so `RD-*` content waits for Slice 5.
- **D538 Slice-1 honest result — the substrate is ELEVEN disconnected components:** the 44 sourced services
  resolve to `CT-01..CT-04`, `CT-05/06/07/09/10`, `CT-11..CT-28`, `CT-35/36` and seven singletons, so most
  traced pairs return `reachable:false` (the shipped default `US`/`E` pair resolves in one rail segment,
  `CTS-R-02`; the `CS`/`E` pair does not resolve at all). That is the CORRECT recorded Slice-1 outcome and
  the precise reason Slice 5 authors the road layer. It is never cured by inventing a service, a window, an
  interchange or an endpoint, and never by parsing a qualitative `dateText` (D526 bars that absolutely).
- **D538 Slice-1 read-only law:** the trace carries `applied:false` and a derived `tracedFriction` that
  NOTHING consumes. `friction` stays the shipped static number in every campaign, so the capped bridge
  (`supply ≤ 7`, `fatigueRelief ≤ 5`, `overall ≤ 2`), the 0.15 `wr.supply` troop-morale weight
  (`src/33-morale.js`), camp and loot are byte-identical. No write to men, morale, casualties, captured,
  victory, score or RNG state from any new path; no field added to `C.conquest`; no save shape moves; the
  build's no-fudge OUTPUT WALL is never relaxed.
- **D538 Slice-1 probe design:** `tools/probe-conquest-supply-plan.mjs` (NEW, filesystem-first) pins the
  contract/law/boundary tokens, the authored-vs-sourced provenance split, the no-new-module/no-data-change
  counts and the Historical negatives, on durable law and history anchors only — never on the current lock
  holder, so a later lane release cannot red it. `tools/probe-conquest-supply.mjs` (NEW, focused) proves the
  seam twice: node-side in a `node:vm` context built from the on-disk `src/115` + `src/114` + `src/61` (so a
  source mutation reds it WITHOUT a rebuild), and in-page against the built deliverable. Both enrol in
  `tools/vet-no-regression.mjs`, which moves the suite 140 → 142; every suite-count pin in `tools/` is
  re-anchored with a chained D538 comment in the same commit.
- **D538 Slice-1 gate contract:** `node --check` on every touched JS/MJS; `node tools/build.mjs` GATE OK;
  artifact readback of the new trace symbols inside the built 61 module; the two new probes; adjacent
  logistics/logistics-rail/bridge/conditioning/conquest-board/conquest-state/conquest-transport/campaign-link/
  full-campaign/auto-resolve/save-slots/command probes; plan probes Mayhem 13/13, War Career 24/24, transport
  12/12, conquest 8/8, doc coherence 5/5; A/B leg 1 = the standing 24-scenario × 8-seed direction battery
  (`tools/sweep-all-battles.mjs`) byte-identical to the `0829d8d` baseline `18f609d07b1190904ec0c11e4ca64675`;
  A/B leg 2 = `tools/probe-full-campaign.mjs` byte-identical to the baseline
  `a38185fd371a7f181250eff3a6cbf76a`; one declared negative bind redding ONLY the node-side containment tooth
  with an md5-proven byte-identical restore; `git diff --check`; docs/ledger sync; commit, push and clean
  parity. The full serialized 140-row battery stays owed at Slice 7. The suite-excluded D528/D530/D532
  research guards are point-in-time proofs pinned to their own heads and are neither rerun nor edited, and
  `tools/probe-desk-pacing-plan.mjs` is a superseded non-enrolled LANE-020 contract probe that is not a gate.
...
- **Resume pointer (D540, retained history):** LANE-022 was `DRIVE` / Claude Code with the complete Slice-2
  acceptance contract committed above and NO runtime byte moved. Slice 2 has since shipped as D541.

- **D541 delivery record — Slice 2, control/service receipts and cuts:** shipped exactly as D540 contracted,
  with two acceptance teeth tightened mid-slice (over-broad, not code defects). The seam is
  `src/61-logistics-rail.js` only. Territory control and per-segment service condition are real state in
  `C.conquest.supply`, read by one pure fail-closed reader (`_lgSupplyView`) and written by exactly two guarded
  mutators (`conquestSupplySetControl` / `conquestSupplySetCondition`) through one guarded store accessor
  (`_lgSupplyStore`). `conquestSupplyTrace` runs a base walk over the full open projection and a live walk
  filtered by cuts and control, resolving to three bounded states: `TRACED` and `SEVERED` flip `applied` and
  `_lgRoute` adopts the clamped `tracedFriction` into `friction` (the single sim-affecting line); `SUBSTRATE_GAP`
  does not — an evidence gap never becomes the player's penalty, and the reason is taught. A cut degrades
  `depotReach` downstream (64 → 52 on the traced US line). The authored opening control map (four Union,
  twenty-four Confederate, eight contested) and the severed-friction ceiling (40) are marked `authored, not
  sourced`. `presLogisticsBlock` gains one guarded conquest supply-line block; the other logistics owners are
  textually unchanged. No module added (manifest 112), no data change (65), `_SAVE_VER` stays 1 (purely
  additive save), `build/base.html` untouched.
- **D541 containment:** the SAME D539 `_lgTraceRuleset` allowlist gate now guards the reader, both mutators,
  the readout block and the trace, so authored control/condition state is structurally unreachable on the
  evidence-gated ruleset and fails CLOSED at the ruleset seam. CONTAINMENT-B owns the gated-ruleset query and
  the allowlist source assertion, and proves eleven closed ruleset shapes plus five closed state paths.
- **D541 A/B — three legs.** Leg 1 (conquest OFF) the 24-scenario × 8-seed direction battery
  `18f609d07b1190904ec0c11e4ca64675` BYTE-IDENTICAL to the re-captured `a613bac` baseline (0 diffs); leg 2
  `probe-full-campaign` `a38185fd371a7f181250eff3a6cbf76a` BYTE-IDENTICAL (0 diffs); leg 3 (conquest ON,
  in-page) US TRACED friction 8→7, US SEVERED 8→40 with depotReach 64→52, CS SUBSTRATE_GAP 12→12 unmoved —
  adjudicated under D92 as accurate-inputs consequences, both columns logged, caps held.
- **D541 bind — Bind D540-B1, exact scope:** mutated the containment allowlist
  (`{ mayhem: 1 }` → `{ mayhem: 1, historical: 1 }`) and ran the focused probe WITHOUT rebuilding; ONLY
  `CONTAINMENT-B` redded (exit 1, 15/16), every other tooth held. Restore byte-identical (`src/61` md5
  `fa03e6a65e76a3e6c994aa026a179f63` pre == post), rebuild returned the identical game md5 at 16/16 green.
- **D541 gates, all green with every artifact read:** build `GATE OK`; focused 16/16 (0 pageerrors, 0
  realErrors); plan 10/10; adjacent logistics-rail 8, logistics 15, bridge 6, conditioning 9, conquest-board
  ...
  `45278110cb73ea4719fa41ffef7682f9` → `4764b1fccb40c473edd871621497f62b`, srcTree
  `08f95d9e9311e90313cc5b7a930f9380` → `5f6d33325fa5feb04f6d5b11e3f5a3b7`, re-anchored at FIVE disk-verified
  sites (three generated-game, two srcTree); suite md5, focused md5, manifest and frozen base HOLD (no probe
  or module added). CF-2 memo authorized by measurement (`conquestTransportNormalized()` 12.10 ms per call was
  the cost, not the 0.14 ms adjacency build); CF-1 re-pinned 5/9 → 6/9 with three structural reds proven to
  predate D539 and left named as LANE-020's.
- **Resume pointer (D541):** LANE-022 is `CONTRACT` / `none` with Slices 1-2 shipped. Slice 3 (repair plus
  finite engineering capacity — where the logged Engineering Corps directive lands load-bearing: pontoons
...
- **D543 delivery record — Slice 3, repair plus finite engineering capacity:** shipped exactly as D542
  contracted. The seam is `src/61-logistics-rail.js` only. Clearing a cut runs through the shipped
  `conquestSupplySetCondition` clear-path, which now consults a pure capacity reader `conquestRepairCapacity`
  over the shipped `C.engineering.levels` (Construction + Pontoon branches, read purely via `_lgEngLevel` — no
  `engInit` mutation on a read path) scaled by the B-5 realism lever, plus a per-pass ledger
  `C.conquest.supply.repair` materialized lazily by `_lgRepairLedger`. A repair the corps cannot afford this
  pass, or lacks the branch for, is REFUSED so the cut stands. `conquestSupplyRepairReport` and the extended
  `_lgSupplyBlockHTML` teach the standing decision; the unwired internal `_lgSupplyRepairReset` is the per-pass
  boundary the Slice 6/7 loop will call. `_lgSupplyView` gained one field, `repairSpent`. One authored config
  constant `_LG_REPAIR` (`authored, not sourced`). No fourth public mutator, no second logistics/engineering
  owner, no second save owner, no new clock (invariant F); the capacity/repair state stays out of the CF-2
  memo. No module added (manifest 112), no data change (65), `_SAVE_VER` stays 1 (purely additive save),
  `build/base.html` untouched.
- **D543 B-5 slider and finite capacity:** repair magnitude reads `fldPresetResolve().attrition` clamped
  `[0.6,1.4]` (the exact `T13 fldEngRealism` lever). A Construction-3 / Pontoon-3 corps has capacity 37 at
  Balanced, 28 at Historian, 40 (clamped) at Arcade. A Construction-1 corps (capacity 10) restores only 2 of
  27 cut rail arteries in one pass and the report marks the pass exhausted — the standing decision. A water
  cut needs a Pontoon Train; Construction alone cannot clear it.
- **D543 containment:** the SAME `_lgTraceRuleset` allowlist gate now also guards `conquestRepairCapacity`,
  `conquestSupplyRepairReport` and `_lgSupplyRepairReset` (all gate through `_lgSupplyView`), so authored
  repair/capacity state is structurally unreachable on the evidence-gated ruleset and fails CLOSED before any
  substrate read. `CONTAINMENT-B` owns the gated-ruleset query and the allowlist source assertion and now
  proves eleven closed ruleset shapes plus eight closed state paths.
- **D543 A/B — three legs.** Leg 1 (conquest OFF) the direction battery `18f609d07b1190904ec0c11e4ca64675`
  BYTE-IDENTICAL to the `5ecc00b` baseline (0 diffs); leg 2 `probe-full-campaign`
  `a38185fd371a7f181250eff3a6cbf76a` BYTE-IDENTICAL (0 diffs); leg 3 (conquest ON, in-page, both columns) the
  US/E rail line SEVERED friction 40 / depotReach 58 → REPAIRED TRACED friction 7 / depotReach 70 (spent 4);
  27 cuts / capacity 10 → 2 restored, exhausted; B-5 37/28/40; water cut restored only with pontoons —
  adjudicated under D92 as accurate-inputs consequences, caps held.
- **D543 bind — Bind D542-B1, exact scope:** mutated the containment allowlist
  (`{ mayhem: 1 }` → `{ mayhem: 1, historical: 1 }`) and ran the focused probe WITHOUT rebuilding; ONLY
  `CONTAINMENT-B` redded (17/18, exit 1), every other tooth held. Restore byte-identical (`src/61` md5
  `1ccd972e5a4f71407d37ddd0d0eb0eaa` pre == post), rebuild returned the identical game md5 at 18/18 green.
- **D543 gates, all green with every artifact read:** build `GATE OK`; focused 18/18 (0 pageerrors, 0
  realErrors); plan 10/10; adjacent logistics-rail 8, logistics 15, bridge 6, conditioning 9,
  engineering-corps 23/23, conquest-board 13/13, conquest-state 15/15, conquest-transport 18/18, campaign-link
  19, auto-resolve 10, save-slots 17/17, command 100, presets 27 — all exit 0, 0 pageerrors; plan probes
  Mayhem 13/13, War Career 24/24, transport 12/12, conquest layer 8/8, doc coherence 5/5; `git diff --check`
  clean. `probe-engineering` FATALs on the reproducible headless-Chrome fonts-stall screenshot timeout
  (environmental — its gameplay steps ran, the engineering surface is byte-unchanged, and engineering-corps
  23/23 cross-validates it); recorded honestly, not a Slice 3 regression. Game
  `4764b1fccb40c473edd871621497f62b` → `98f3feaf0de89b3b47eda6b1347dacd0`, srcTree
  `5f6d33325fa5feb04f6d5b11e3f5a3b7` → `c4fc64ebe6d49d9cdfc79885b4c05d8b`, re-anchored at FIVE disk-verified
  sites (three generated-game, two srcTree); suite md5, focused md5, manifest and frozen base HOLD (no probe
  or module added).
- **Resume pointer (D543):** LANE-022 is `CONTRACT` / `none` with Slices 1-3 shipped. Slice 4 (blockade/sea
  edge — blockade state gates the two bounded sea services as the Confederate import edge, reusing shipped
...
- **D545 delivery record — Slice 4, the blockade / sea edge:** shipped exactly as D544 contracted. The seam is
  `src/61-logistics-rail.js` only. For a CS carrier `conquestSupplyTrace` builds a source list — the interior
  depot FIRST (depot-preferred, so every shipped route is byte-identical), then every held coastal port that
  originates a sourced sea service (the from-end of a sea-mode edge — CT-12 Savannah); the first source whose
  open-graph walk reaches the target wins, else it falls back to the depot. A pure reader `_lgSeaImportOpen(C)`
  reads the shipped `C.blockade.portsOpen`: open (`portsOpen > 0`) lets the import carry, sealed (`portsOpen 0`)
  closes it so a base-reachable import-sourced front SEVERS at the ceiling (40) rather than becoming a
  `SUBSTRATE_GAP`. No fourth mutator, no second logistics/blockade owner, no new save owner, no new clock, no
  authored constant, no module added (manifest 112), no data change (65), `_SAVE_VER` stays 1 (the sea edge
  reads `C.blockade`, so nothing new serializes), `build/base.html` frozen.
- **D545 containment:** the sea-import logic lives inside `conquestSupplyTrace`, which fails CLOSED at the D539
  `_lgTraceRuleset` allowlist gate before any board read. `CONTAINMENT-A` gained the open-ruleset sea-import
  assertion (`CS→CT-11` `TRACED` via `CTS-S-01`); `CONTAINMENT-B` stays the ONE tooth owning the gated-ruleset
  query and the allowlist source assertion, and now proves a CS sea-import query is null on the gated ruleset.
- **D545 A/B — three legs.** Leg 1 (conquest OFF) the direction battery `18f609d07b1190904ec0c11e4ca64675`
  BYTE-IDENTICAL to the `caf3855` baseline (0 diffs); leg 2 `probe-full-campaign`
  `a38185fd371a7f181250eff3a6cbf76a` BYTE-IDENTICAL (0 diffs); leg 3 (conquest ON, in-page, both columns) at
  caf3855 both CS coast targets were `SUBSTRATE_GAP`; in this build blockade OPEN → `CS→CT-11` `TRACED` friction
  8 via `CTS-S-01:sea`, Western `CS→CT-20` `TRACED` friction 16, blockade SEALED → both `SEVERED` friction 40
  (`severedBy CTS-S-01`) — adjudicated under D92 as accurate-inputs consequences, caps held.
- **D545 bind — Bind D544-B1, exact scope:** mutated the containment allowlist
  (`{ mayhem: 1 }` → `{ mayhem: 1, historical: 1 }`) and ran the focused probe WITHOUT rebuilding; ONLY
  `CONTAINMENT-B` redded (19/20, exit 1), every other tooth held. Restore byte-identical (`src/61` md5
  `4aa935355487feec3a0780abba7316b4` pre == post), rebuild returned the identical game md5 at 20/20 green.
- **D545 gates, all green with every artifact read:** build `GATE OK`; focused 20/20 (0 pageerrors, 0
  realErrors); plan 10/10; adjacent logistics-rail 8, logistics 15, bridge 6, conditioning 9, blockade 11,
  conquest-board 13/13, conquest-state 15/15, conquest-transport 18/18, campaign-link 19, auto-resolve 10,
  save-slots 17/17, command 100, presets 27 — all exit 0, 0 pageerrors; plan probes Mayhem 13/13, War Career
  24/24, transport 12/12, conquest layer 8/8, doc coherence 5/5; `git diff --check` clean. Game
  `98f3feaf0de89b3b47eda6b1347dacd0` → `0a5286c3b79c8011a6903ceb23772d80`, srcTree
  `c4fc64ebe6d49d9cdfc79885b4c05d8b` → `7bcb0579d4e432950897500e7f0e5846`, re-anchored at FIVE disk-verified
  sites (three generated-game, two srcTree); suite md5, focused md5, manifest and frozen base HOLD.
- **Resume pointer (D545):** LANE-022 is `CONTRACT` / `none` with Slices 1-4 shipped. Slice 5 (the authored
  Mayhem road layer, seeded on `RD-SI06`/`RD-SI13` as sourced labelled exemplars of road FAILURE with the rest
  openly authored and visibly marked, structurally barred from Historical) needs its own committed contract
  section and DRIVE take here before any runtime edit. D545 replaces D543 as the ARC 7 product head. ARC 7
  Historical transport movement, Historical roads, the four `CTI-*` faces and E46 remain blocked.
- **D546 Slice-5 source-vs-design-law finding (surfaced, default-refute citation pass, 2026-07-23):** before authoring
  the two sourced seeds, each element of the design-law §4 Slice-5 sentence was verified against the original
  Official Records text (Internet Archive item `warofrebellion014701rootrich`, full `_djvu.txt`, Report No. 32 read
  back at printed pages 336-342) and the Platter diary (Digital Library of Georgia `dlg_zlpd_ccp001`). BOTH rows
  survive as **Verified**, and the D532 locators were genuinely read back, not manufactured. FOUR wording defects in
  the design-law sentence were found and are corrected in the authored copy rather than propagated: (1) Corse crossed
  the **Lawtonville roads**, not the town of Lawtonville — the town was traversed by the XX Corps and by Logan's First
  and Third Divisions, not by Corse's Fourth; (2) Corse crossed **Coosawhatchie Swamp**, an inland watercourse, NOT
  the Charleston and Savannah Railroad depot town of that name — left uncorrected this would put a road segment on the
  wrong side of the board; (3) "miles of corduroy" overstates the source, whose own construction is men "marching for
  miles with fence rails upon their shoulders with which to corduroy the swamps and quicksand roads"; (4) `RD-SI13`'s
  midnight halt was at **Johnson's plantation beyond Laurel Hill**, not at Laurel Hill, and the rain continued "nearly
  the entire day", not all day. Two further findings are recorded and NOT acted on here: Platter was adjutant of the
  81st Ohio, which was Second Brigade of Corse's OWN Fourth Division, so the D532 "2/2 independent families" floor is
  weaker in command-independence than the phrase implies — cured at zero cost by citing Logan's cross-command Report
  No. 11 (pp. 224-225) alongside Corse, which this slice does; and the FROZEN D511 register row carries Platter's
  Buford's Bridge where Corse's report says Rivers' Bridge, a live contradiction that is SURFACED, not silently
  reconciled, and that touches no design-law sentence and no authored road here. `RD-SI13`'s locator is authored as
  **p. 340** (the whole claim is on that page); the D532 packet row and its research guard keep "printed pp. 340-342"
  and are NOT edited, so LANE-019 stays unrewritten.
- **D546 exact Slice-5 objective:** author the Mayhem road layer, so the eleven disconnected components of the sourced
  substrate become ONE connected board and every front can be supplied overland at a real, costly price. Roads are
  mode `road` at the shipped `_LG_TRACE_COST` cost 6 — double rail, triple inland water — so a road route is a genuine
  but expensive fallback and the §5 mode economics finally bite. Non-conquest play stays byte-identical; the bounded
  channel is unchanged; no new combat channel is created.
- **D546 exact Slice-5 seam (`src/61-logistics-rail.js` only):** ONE new authored constant `_LG_ROADS` and one pure
  merge inside `_lgTraceBase`. `_lgTraceProject` / `_lgTraceGraph` stay SOURCED-ONLY and byte-unchanged, so the
  eleven-component / 44-service proof of the sourced substrate remains exactly as D539 shipped it and keeps working as
  the substrate-immutability tooth. The roads are merged into the memoized base projection only — `base.adj` gains the
  road edges and `base.services` gains the road ids with mode `road` — which is CORRECT for the CF-2 memo because an
  authored road graph is a static, control- and condition-INDEPENDENT input, exactly the class the memo is contracted
  to hold (a CONSCIOUS decision, proved by the unchanged memo-equals-cold tooth against the same merged projection).
- **D546 Slice-5 authored Mayhem content (openly authored game content, never a Historical claim):** ONE constant
  `_LG_ROADS` carrying seventeen rows — TWO sourced, labelled exemplars and FIFTEEN openly authored segments. Every
  authored row is marked `authored, not sourced` in the seam exactly as the D539/D540/D542 constants are; every
  sourced row carries `provenance: "Verified"` and an exact `source` locator. Roads are bidirectional (a wagon road
  works both ways) and their ids are chosen so that, for the same destination, `_lgTraceEdgeOrder` sorts EVERY sourced
  `CTS-*` service ahead of every road — so a road can never displace an equal-hop sourced segment and the shipped
  rail/water routes stay exactly as they were.
- **D546 Slice-5 sourced seeds (the two D532 cured rows; citation-grade, verified default-refute):**
  `RD-SI06` ships as a REAL board edge `CT-12` (Savannah-Coastal Georgia) to `CT-11` (Charleston-Port Royal
  Lowcountry) — high confidence at BOTH ends: the column crossed the Savannah at Sister's Ferry and its entire
  documented middle (Robertsville, the Lawtonville roads, Coosawhatchie Swamp, Whippy Swamp) lies inside the CT-11
  lowcountry. It teaches road FAILURE in the source's own words: the most inclement weather and roads "which under
  other circumstances would have been looked upon as almost impassable", the whole division "organized into a
  temporary pioneer corps", men marching for miles carrying fence rails to corduroy swamps and quicksand roads
  "before a wagon could pass over them" and labouring in water to their waists, especially at Whippy Swamp, and the
  11 February pontoon crossing at Binnaker's Bridge on the South Edisto. Source: Official Records Series I, vol. 47,
  pt. I, No. 32 (Bvt. Maj. Gen. John M. Corse, Fourth Division, XV Corps), pp. 337-338, corroborated at a second
  echelon by ibid. No. 11 (Maj. Gen. John A. Logan, XV Corps), pp. 224-225.
  `RD-SI13` ships as a sourced, labelled exemplar with **NO board endpoints and therefore NO edge** — the honest,
  D503-consistent result, because its documented route (Cheraw, S.C. on the Great Pee Dee, through Laurel Hill in the
  sandhills, toward Fayetteville on the Cape Fear) has NO anchor in any of the 36 territories at its origin, and the
  default-refute pass explicitly REFUTED the CT-11 and CT-13 readings. Forcing an endpoint would invent geography, so
  it does not get one. A null-endpoint row projecting no edge is the SHIPPED substrate's own behaviour (`CTS-S-02`
  Wilmington is a single-territory sourced service that projects no graph edge), so this needs no new mechanism. It
  teaches the second and different road failure — a road passable at 10 a.m. and impassable by midnight: Laurel Hill
  reached via the telegraph road at 10 a.m. on 9 March 1865, then flooded and almost impassable roads under a violent
  rain-storm that continued nearly the entire day, men and animals completely exhausted, and a forced midnight halt at
  Johnson's plantation. Source: ibid. No. 32, p. 340, corroborated by the Platter diary (Hargrett MS 2570, DLG
  `dlg_zlpd_ccp001`), 9 March 1865.
- **D546 Slice-5 authored network (fifteen segments, each a real wagon-road corridor, none a sourced claim):**
  `RDA-01` CT-03 to CT-05 (the Fredericksburg/Telegraph road, the most-marched corridor of the Eastern war) ·
  `RDA-02` CT-04 to CT-07 (the Valley Pike to Staunton and the Lynchburg road) · `RDA-03` CT-05 to CT-08 (the
  Peninsula roads) · `RDA-04` CT-07 to CT-21 (the Great Valley road through southwest Virginia) · `RDA-05` CT-10 to
  CT-11 (the Carolina coastal road, Wilmington-Georgetown-Charleston) · `RDA-06` CT-25 to CT-34 (the Vicksburg-Monroe-
  Shreveport Trans-Mississippi lifeline) · `RDA-07` CT-34 to CT-33 (the Red River valley road) · `RDA-08` CT-34 to
  CT-35 (Shreveport to Marshall) · `RDA-09` CT-34 to CT-32 (the Camden road to Little Rock) · `RDA-10` CT-32 to CT-31
  (the Arkansas River road) · `RDA-11` CT-32 to CT-30 (the Wire Road) · `RDA-12` CT-30 to CT-29 (the Rolla road) ·
  `RDA-13` CT-29 to CT-19 (St. Louis-Cape Girardeau-Cairo) · `RDA-14` CT-02 to CT-18 (the National Road over the
  Alleghenies) · `RDA-15` CT-18 to CT-19 (the Ohio valley road, Louisville-Paducah-Cairo). Measured result: the
  sourced projection stays at ELEVEN components and 44 services, while the roaded base projection is ONE component of
  36 territories and 60 services (44 sourced + 16 road edges; `RD-SI13` projects none).
- **D546 exact Slice-5 source-selection amendment (a defect Slice 5 exposes in the Slice-4 rule, fixed here):**
  D544/D545 selected the first source whose open walk reached, depot preferred — correct then, because the depot
  reached almost nothing and depot-preference was byte-identity scaffolding. With roads the depot reaches nearly
  everything, so first-wins would select a STRICTLY WORSE line: measured, `CS` to `CT-11` would fall from the
  1-segment sea import at friction 8 to a 4-segment interior road route at friction 19 purely because of preference
  order, silently destroying the Slice-4 blockade lever. The rule therefore becomes: among the sources that can
  ACTUALLY source (the interior depot always; a coastal sea-import port only while the blockade is open), take the one
  whose open walk has the LOWEST total mode cost, the depot winning ties because it is first. If no available source
  reaches but a blockade-closed one would have, the Slice-4 blockade-`SEVERED` branch is kept exactly, so a genuinely
  sea-only front still severs on the blockade and never degrades to `SUBSTRATE_GAP`. When a closed import WOULD have
  been cheaper than the interior line actually used, the trace carries `importClosed: true` and the reason names the
  sealed ports, so the player is told why they are on the long road. A uniform-cost walk was BUILT AND MEASURED as the
  alternative and produced results identical to the shipped breadth-first walk on every measured pair, so
  `_lgTraceWalk` is deliberately NOT changed — the cheapest-source fix is the whole of the correction.
- **D546 exact Slice-5 cut/repair law (reusing Slice 2 and Slice 3 generically; no new owner, no new field):** design
  law §5 says roads are destroyed by weather and design law §4 Slice 3 promised that "clearing restores a road"; Slice
  3 could only implement that against rail and water because roads did not exist. Slice 5 lands it. Road segments are
  cut and cleared through the SHIPPED `conquestSupplySetCondition` over the SAME `C.conquest.supply.cut` map, so there
  is no fourth mutator, no new namespace and no new save field. The ONE change required is the `_lgSupplyView`
  sanitizer's id pattern, which today admits only `CTS-[RWS]-\d\d` and therefore silently rejected every road cut in
  the pre-authoring instrumentation; it widens to admit the authored road id namespace as well, and NOTHING else about
  the sanitizer moves — it still fails CLOSED on a malformed payload, and a road id that is not in the base projection
  is still refused by the mutator's `base.services` membership check. `_LG_REPAIR.cost` gains a `road` entry: clearing
  a blocked road is the cheapest repair in the game and is the Construction Corps' work, never the Pontoon Train's.
- **D546 Slice-5 save law (invariant E, the GEA-12/D447 precedent):** `_LG_ROADS` is a module-scoped AUTHORED
  CONSTANT, not player state, so `C.conquest` gains NO field and there is nothing new to serialize. A road cut rides
  the existing `cut` map as one more key, so none of the seven functions hashed in `tools/save-shape.json` is touched
  and `_SAVE_VER` stays **1** as a CONSCIOUS decision. A pre-Slice-5 legacy save must load, round-trip and
  re-serialize BYTE-IDENTICALLY — that tooth is gating, and it is gating precisely because the sanitizer's accepted
  key set widens.
- **D546 exact Slice-5 containment seam (the declared bind target):** `_LG_ROADS` is consumed ONLY inside
  `_lgTraceBase`, which is reachable only after `_lgConquestKind` and the D539 `_lgTraceRuleset` allowlist gate have
  both passed, so the authored road graph is structurally unreachable on the evidence-gated ruleset and fails CLOSED
  at the ruleset seam BEFORE any board or transport-evidence read. `CONTAINMENT-A` gains the open-ruleset road
  assertions; `CONTAINMENT-B` stays the ONE tooth owning the gated-ruleset query and the allowlist source assertion —
  a bind that reds two teeth is too broad (the D539/D541/D543/D545 lesson). The Historical zero-road negative stays
  EXACT: `data/conquest-transport-evidence.json` keeps `roadStatus` `ROAD_REQUIRES_BOUNDED_SOURCE_PASS` with no
  `roadServices` key, `src/115` and `src/114` stay byte-frozen, `conquestTransportPhysicalServices` still returns zero
  road-mode services, and no authored road is ever written into the evidence pack or shares its namespace. New
  Orleans-origin, `CT-36`, the D503 endpoint quarantine, the Potomac/operation-composition/Sherman-chain negatives,
  the permanent unassignment of Boonville, Arrow Rock and Glasgow, and `CTI-01`..`CTI-04` staying
  `INTERCHANGE_WINDOW_UNADJUDICATED` for Historical all remain untouched and binding.
- **D546 Slice-5 bounded-channel law (unchanged from shipped):** `logisticsBridgeBonus` keeps its caps
  (`supply <= 7`, `fatigueRelief <= 5`, `overall <= 2`) and `wr.supply` keeps its 0.15 troop-morale weight in
  `src/33-morale.js`. The road layer is a bounded CONDITIONING INPUT feeding the already-capped bridge through the
  single shipped `_lgRoute` adoption line: it never writes casualties, morale, victory, score or RNG, and Slice 5
  creates NO new combat channel.
- **D546 Slice-5 mode-economics law (design law §5 and the point of the slice):** a road route must be genuinely
  available where rail and water are not, and genuinely worse. Measured on the authored board: `CS` to its Eastern
  front resolves in ONE road segment at friction 10 where it was an un-applied `SUBSTRATE_GAP`; `CS` to Little Rock
  and to Marshall resolve at friction 33 across seven segments, three of them road — a real line that costs nearly
  the severed ceiling, which is exactly the teaching; and a six-segment all-road route would cost 40, the severed
  ceiling itself, so the road never becomes a free pass. The `SUBSTRATE_GAP` state is NOT retired — it remains the
  first-class result for a pair no sourced or authored line links, and its reason text is corrected because the road
  layer is no longer "not built yet".
- **D546 Slice-5 probe design (three CONTRACTED tooth reshapes, each strictly tighter, none weakened):**
  `tools/probe-conquest-supply.mjs` grows from 20 teeth and no probe is added, so the suite STAYS 142 and the twelve
  suite-count, two suite-md5 and two focused-md5 pins do NOT move. (1) The `NO NEW AUTHORITY` tooth today forbids the
  bare token `RD-` anywhere in the trace output; Slice 5 legitimately puts `RD-SI06` there, so the blanket ban is
  REPLACED by a strictly tighter positive assertion — every `RD-`/`RDA-` id appearing in the seam or its output must
  be one of exactly the two cured `RD-SI` ids or the authored `RDA-` namespace, the four UNRESOLVED `RD-E1*` rows stay
  forbidden outright, and `roadService`/`dateText`/`historicalEligibility`/`eligibility`/`window`/`legalNow`/
  `availability` stay forbidden exactly as before. (2) The identical blanket `RD-SI` ban in
  `tools/probe-conquest-supply-plan.mjs` is replaced the same way, and that probe's `authored, not sourced` marker
  count is re-pinned CONSCIOUSLY for the one added authored constant. (3) The `SUBSTRATE GAP IS FIRST-CLASS` tooth
  asserts that a cross-component target "must stay an un-applied gap until the road layer lands" and that the reason
  says the road layer is "not built yet" — both are the pre-Slice-5 truth by construction, so the tooth becomes THE
  ROAD LAYER CLOSES THE SUBSTRATE GAP: the exact pairs that were gaps at `ca6b63e` now resolve over the authored
  roads, the `SUBSTRATE_GAP` branch and its no-penalty law remain present and correct as the defensive result, and the
  reason text is re-pinned to its corrected wording. Additionally the `TRACE CORRECTNESS` tooth keeps its ELEVEN-
  component and 44-service assertions on the SOURCED projection unchanged (that is why the merge lives in
  `_lgTraceBase` and not in `_lgTraceProject`) and gains the roaded-base assertions (ONE component of 36, 60 base
  services); the `CF-2 MEMO` tooth compares the memo against a cold recompute of the SAME merged projection; the SAVE
  SHAPE tooth gains road-cut round-trip and malformed-road-id fail-closed cases; and ONE new step proves the road
  layer end to end — the two sourced seeds' provenance/source/teaching fields, the fifteen authored markers, a
  road-only route and its higher friction, a road cut degrading a route and a Construction-Corps repair restoring it,
  the cheapest-source selection preserving the Slice-4 sea import at friction 8 open and falling back to the interior
  road at friction 19 sealed with `importClosed` set, and that no road ever displaces an equal-hop sourced segment.
  Every tooth stays anchored on durable law/history, never on the current lock holder (D391).
- **D546 Slice-5 gate contract:** `node --check` on every touched JS/MJS; `node tools/build.mjs` GATE OK; artifact
  readback of the new road symbols inside the built 61 module region; the two LANE-022 probes; adjacent
  logistics-rail, logistics, bridge, conditioning, engineering-corps, blockade, conquest-board 13/13, conquest-state
  15/15, conquest-transport 18/18, campaign-link, auto-resolve, save-slots 17/17, command and presets probes; plan
  probes Mayhem 13/13, War Career 24/24, transport 12/12, conquest layer 8/8, doc coherence 5/5; **A/B leg 1** — the
  standing 24-scenario x 8-seed direction battery byte-identical to the `ca6b63e` baseline
  `18f609d07b1190904ec0c11e4ca64675`; **A/B leg 2** — `probe-full-campaign` byte-identical to
  `a38185fd371a7f181250eff3a6cbf76a`; **A/B leg 3** — the conquest-ON leg, which WILL move substantially as the
  eleven components join: both columns logged for every measured row, the direction stated, and every movement
  adjudicated under D92 as an accurate-inputs consequence, never tuned toward a preferred number and never gated on an
  output. The largest movements are known in advance from the pre-authoring instrumentation and are contracted here so
  they cannot be discovered and rationalised after the fact: `US` to its Western and Trans-Mississippi fronts moves
  from an un-applied `SUBSTRATE_GAP` to `SEVERED` at the ceiling 40, because Nashville and Little Rock are Confederate
  in the authored 1861 opening control map and an enemy-held destination severs the line under the SHIPPED D540/D541
  traversal law — the Union genuinely had no supply line into Confederate Tennessee in 1861, and the state flips to
  `TRACED` as the player takes the ground, which is the entire point of a conquest layer; `US` to its Northern front
  becomes `TRACED` at 19 over the National Road and the Ohio valley road; `CS` to its Eastern front becomes `TRACED`
  at 10; and the blockade lever on `CS` to `CT-11` becomes 8 open / 19 sealed instead of 8 / 40, because a real
  interior alternative now exists. ONE declared negative bind on the containment allowlist redding ONLY
  `CONTAINMENT-B`, with an md5-proven byte-identical restore; `git diff --check`; docs/ledger sync; commit, push and
  clean parity. Five pin sites re-anchor (three generated-game, two srcTree); suite/focused/manifest/base pins hold.
  The full serialized 142-row battery stays owed at Slice 7. The suite-excluded D528/D530/D532 research guards are
  point-in-time proofs pinned to their own heads and are neither rerun nor edited, and `tools/probe-desk-pacing-plan.mjs`
  is a superseded non-enrolled LANE-020 contract probe that is not a gate.
- **Resume pointer (D546):** LANE-022 is `DRIVE` / Claude Code with the complete Slice-5 acceptance contract committed
  above and NO runtime byte moved. Implement the seam in `src/61-logistics-rail.js` only (plus the two LANE-022 probes
