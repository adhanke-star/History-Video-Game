...
function _cxEntryHTML(en) {
  ...
  return ''
    ...
    +     ((typeof usctImageHtml === "function" ? usctImageHtml(en.id) : "")
       || (typeof leaderImageHtml === "function" ? leaderImageHtml(en.id) : ""))   /* H1: a PD photo at the head of the expanded card — the USCT image (D135) wins where one exists, else the leader/figure portrait (D136); "" when neither -> the card is byte-identical */
