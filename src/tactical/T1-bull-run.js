...
function fldScenarioRegistry() {
  ...
  try {
    if (typeof GAME_DATA !== "undefined") {
      ...
      if (GAME_DATA.olustee && GAME_DATA.olustee.olustee) R.olustee = GAME_DATA.olustee.olustee;   // D470 LANE-016 (the D465 spec is the law): Olustee / Ocean Pond - the meeting engagement in the Florida pine corridor, February 20, 1864 (single-phase; an honest Union defeat from true inputs; the aftermath killing of Black wounded is taught with its contested scale honest and judged through the D457 no-quarter machinery, never played)
      if (GAME_DATA["fort-pillow"] && GAME_DATA["fort-pillow"].fortPillow) R.fortPillow = GAME_DATA["fort-pillow"].fortPillow;   // D463 LANE-013 P4 (the D455 SS3 row 6 unlock): Fort Pillow - Forrest's assault on the bluff fort, April 12, 1864 (single-phase attacker-seize; the battle ends at the works - the massacre is taught and judged through the D457 no-quarter machinery, never played)
      if (GAME_DATA.wilderness && GAME_DATA.wilderness.wilderness) R.wilderness = GAME_DATA.wilderness.wilderness;   // D393 Overland/attrition lane: the Wilderness - CS attacks the Brock Road / Orange Plank Road junction through the thickets (single phase; US defender holds the road south)
      ...
      if (GAME_DATA["cold-harbor"] && GAME_DATA["cold-harbor"].coldHarbor) R.coldHarbor = GAME_DATA["cold-harbor"].coldHarbor;   // D442 Overland/attrition lane: Cold Harbor - the June 3, 1864 dawn assault (single-phase doomed frontal assault: II/VI/XVIII into the entrenched line; the honest Union failure; fog OFF - the killing ground is works + enfilade angles + narrow gaps, never a fog buff)
      ...
      if (GAME_DATA.crater && GAME_DATA.crater.crater) R.crater = GAME_DATA.crater.crater;   // D469 LANE-015 (the D464 spec is the law): The Crater - the assault out of the mine breach at Petersburg, July 30, 1864 (single-phase; the battle ends on the field - the killing of surrendering USCT that followed is taught and judged through the D457 no-quarter machinery, never played)
...
function fldScenarioMenuRank(id) {
  var order = { bullrun1: 10, crossKeysPortRepublic: 12, gainesMill: 15, malvernHill: 18, antietam: 20, fredericksburg: 30, chancellorsville: 35, gettysburg: 40, newMarketHeights: 45, fortDonelson: 48, elkhornTavern: 49, shiloh: 50, stonesRiver: 52, vicksburg: 55, chickamauga: 60, chattanooga: 65, olustee: 65.5, fortPillow: 66, wilderness: 67, spotsylvania: 68, coldHarbor: 68.5, petersburgAssaults: 69, kennesaw: 70, atlanta: 71, crater: 71.5, cedarCreek: 72, franklin: 75, nashville: 80, fiveForks: 85 };   // D436: atlanta slots at 71 — Jul 22 1864, after Kennesaw (Jun 27) and before Cedar Creek (Oct 19); the documented chronology convention. D442: coldHarbor slots at 68.5 — Jun 3 1864 sits between Spotsylvania (May 12, rank 68) and the Petersburg initial assaults (Jun 15, rank 69); the comparator is numeric, and renumbering the table would churn every later pin for no history gain (the spec §7 documented non-integer exception). D469: crater slots at 71.5 — Jul 30 1864 sits between Atlanta (Jul 22, rank 71) and Cedar Creek (Oct 19, rank 72); the same documented D442 non-integer exception (the D464 spec §2 pins it). D470: olustee slots at 65.5 — Feb 20 1864 sits between Chattanooga (Nov 1863, rank 65) and Fort Pillow (Apr 12 1864, rank 66); the same exception (the D465 spec §2 pins it).
