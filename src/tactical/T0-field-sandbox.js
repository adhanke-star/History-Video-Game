...
function fldInitSim(opts) {
  ...
  var _campVal = (opts.campaign && typeof _fldCamp === "function") ? _fldCamp() : null;
  __FIELD.playerSide =
    ...
    : (_campVal && _campVal.side === "CS") ? "CS" : "US";
...
function fldOnOver() {
  ...
  if (__FIELD.phases && typeof _fldPhasesEndHtml === "function") { try { scNote = (_fldPhasesEndHtml() || "") + scNote; } catch (eP) { if (typeof console !== "undefined" && console.warn) console.warn("[T8] _fldPhasesEndHtml error:", eP); } }
  ...
  if (a) a.addEventListener("click", function () { var sd = (__FIELD.seed + 7) >>> 0, lo = __FIELD._launchOpts, k = __FIELD.rendererKind, sc = __FIELD.scenario, ps = __FIELD.playerSide; fldExit(true); var _o; if (lo) { _o = {}; for (var _k in lo) if (Object.prototype.hasOwnProperty.call(lo, _k)) _o[_k] = lo[_k]; _o.seed = sd; } else { _o = { renderer: k, seed: sd, scenario: sc, playerSide: ps }; } fldLaunchSandbox(_o); });
...
function fld3dInit() {
  ...
  __FIELD.raycaster = new T.Raycaster(); __FIELD.ground = null; __FIELD._terrainObjs = [];
  fld3dBuildTerrain();
...
function fld3dBuildTerrain() {
  ...
  if (!__FIELD._terrainObjs) __FIELD._terrainObjs = [];
  __FIELD._terrainObjs.push(mesh);
  // objective marker
  ...
  __FIELD._terrainObjs.push(ring);
  ...
  for (var wq = 0; wq < _ws.length; wq++) {
    ...
    __FIELD._terrainObjs.push(wall);
  }
  ...
  if (t.woods) for (var w = 0; w < t.woods.length; w++) {
    ...
    __FIELD._terrainObjs.push(im);
  }
}
/* dispose + rebuild all 3D terrain objects (ground mesh, objective ring, walls, woods, markers).
   Called on phase advance so the new phase's terrain/objective renders correctly in 3D mode. */
function fld3dRebuildTerrain() {
  if (!__FIELD.scene || !window.THREE) return;
  var objs = __FIELD._terrainObjs || [];
  for (var i = 0; i < objs.length; i++) {
    var obj = objs[i];
    if (obj.geometry && obj.geometry.dispose) obj.geometry.dispose();
    if (obj.material) { var m = obj.material; if (Array.isArray(m)) { for (var j = 0; j < m.length; j++) m[j] && m[j].dispose && m[j].dispose(); } else if (m.dispose) m.dispose(); }
    __FIELD.scene.remove(obj);
  }
  __FIELD._terrainObjs = [];
  __FIELD.ground = null;
  fld3dBuildTerrain();
}
  ...
  for (var i = 0; i < t.markers.length; i++) {
    ...
    if (mk.path && mk.path.length > 1) {
      ...
      for (var p = 1; p < mk.path.length; p++) {
        ...
        if (__FIELD._terrainObjs) __FIELD._terrainObjs.push(seg);
      }
      ...
      if (__FIELD._terrainObjs) __FIELD._terrainObjs.push(m2);
    }
  }
}
...
function fld3dDispose() {
  ...
  __FIELD.scene = null; __FIELD.camera = null; __FIELD.controls = null; __FIELD.renderer = null; __FIELD.groups = null; __FIELD._u3d = null; __FIELD.ground = null; __FIELD._terrainObjs = null;
