...
**Single source of truth: [`START-HERE.md`](../START-HERE.md) — and `CLAUDE.md` / `AGENTS.md` carry the full read-order + standards.** Read them before suggesting nontrivial changes. You assist *inline*; you do not run the build/probe gate, so your job is to propose edits that fit the architecture and to flag what a human must verify. D145/D286 model routing applies: Copilot/helper output is bounded assistance; the top model (Claude Opus 4.8 — `claude-opus-4-8` / `opus[1m]` at xhigh — in Claude Code, 5.5 in Codex) owns architecture, important code, historical judgment, UX/accessibility direction, bug hunts, and final integration.
