...
- [ ] **H5 — THE UG:G-FIDELITY MODERN-ENGINE PUSH** ‹⭐ Aaron, 2026-06-25 — LOCKED direction; memory `civilwar-modern-engine-ugg-fidelity` + `civilwar-terrain-readability`›: **the modern (non-hex) battle engine must look AND play better than Ultimate General: Gettysburg (UG:G) on PS4.** Two engines, two jobs (do NOT conflate): the **hex/box map IS Classic** — keep as-is; the **modern engine has NO hex boxes** — fluid UG:G-style motion with **drag-click-and-direct** movement & charge orders, formation facing, and a **fortify/entrench option**. Build order (AskUserQuestion 2026-06-25):
      - **H5-i1 — MOVEMENT & ORDERING FEEL (FIRST):** smooth drag-to-move/charge, click-direct orders, formation facing, fortify/entrench commands — zero grid snap; make the troops move and obey exactly like UG:G. (Base: the real-time engine `fldLaunch*` / `tactical/T*` drag-to-maneuver; this is GAMEPLAY/engine work — movement/order changes that alter the sim are NOT byte-identical and must be vetted as gameplay, not under D74.)
      - **H5-i2 — TERRAIN READABILITY:** explicit high/low-ground identification (elevation legend + on-hover readout), a **shading↔lines TOGGLE** (default **HILLSHADE relief**; toggle to contour-lines+labels and color-by-height), and **all topography distinguished** (woods/fields/ridges/hills/rivers-fords/walls/roads/swamp/town/fort). Presentation-only → honor D74 byte-identity (prove via probe-presets + probe-phased-ab); the toggle is a settings lever like the realism sliders. CVD-safe palettes. (Base: `fldBuildTerrain`/`fld3dBuildTerrain` + T12-3d-readability + T18-render-richness + `tcDecorateMap`.)
      - **H5-i3 — VISUAL FIDELITY:** the **polished HYBRID** look (3D terrain + stylized troop markers/flags), made far richer — terrain, lighting, smoke, unit detail — to surpass UG:G's appearance. (Extends H3; reuse the run-h PBR/HDRI/post-FX.)
      - **H5-i4 — FORTIFICATIONS DEPTH:** deepen the entrench/fortify/abatis/redoubt orders + their visible earthworks (ties to T13-engineering + the B-5 realism sliders).
