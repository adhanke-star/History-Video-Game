...
3. **Group 3 — H0 batch/release gate (NEXT):** run the completed-H0 batch/release verification checkpoint as its own long-grace browser-gate chat. Do not weaken probes. Inspect JSON/stdout for `ok=false`, `FATAL`, `pageerrors`, filtered errors, and real errors.
