...
- [~] **"The Soldier's Story" (D93, Aaron) — build AFTER the C+E roadmap; unified with the journey mode above.**
      ...
      **detail cards** (DATA, reuses teaching cards, NO engine change — brigades stay one marker per the D68
      ...
      persistence, battle association, status history, promotion count, and after-action report tie-in. **D154 shipped
      the first narrow citation-grade named-person slice:** Elisha Hunt Rhodes replaces one generated Bull Run private
      row with a sourced Verified record and portrait. **Still open:** citation-grade named bios/portraits/unit detail
      at scale for every identifiable person, richer start-anywhere career trajectory, and later interpersonal hooks.
      (Memory `civilwar-soldiers-story-pillar`; DECISIONS D93/D148/D150/D151/D154.)
