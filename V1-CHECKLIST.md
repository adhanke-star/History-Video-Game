...
- [ ] **H5 — THE UG:G-FIDELITY MODERN-ENGINE PUSH** ‹⭐ Aaron, 2026-06-25 — LOCKED direction; memory `civilwar-modern-engine-ugg-fidelity` + `civilwar-terrain-readability`›: **the modern (non-hex) battle engine must look AND play better than Ultimate General: Gettysburg (UG:G) on PS4.** Two engines, two jobs (do NOT conflate): the **hex/box map IS Classic** — keep as-is; the **modern engine has NO hex boxes** — fluid UG:G-style motion with **drag-click-and-direct** movement & charge orders, formation facing, and a **fortify/entrench option**. Build order (AskUserQuestion 2026-06-25):
      ...
      - **H5-i3 — TERRAIN READABILITY:** explicit high/low-ground identification (elevation legend + on-hover readout), a **shading↔lines TOGGLE** (default **HILLSHADE relief**; toggle to contour-lines+labels and color-by-height), and **all topography distinguished** (woods/fields/ridges/hills/rivers-fords/walls/roads/swamp/town/fort). Presentation-only → honor D74 byte-identity (prove via probe-presets + probe-phased-ab); the toggle is a settings lever like the realism sliders. CVD-safe palettes. (Base: `fldBuildTerrain`/`fld3dBuildTerrain` + T12-3d-readability + T18-render-richness + the new T21 relief + `tcDecorateMap`.)
