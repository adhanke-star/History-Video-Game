...
Shipped exactly as D538 contracted, from the clean pushed `f4df08e` boundary. The GEA-11 defect closes at
its named seam: `_lgRoute` keeps its shipped six keys computed exactly as before and gains one guarded tail
for a conquest carrier on the open ruleset, backed by eight pure helpers and four authored constants in
`src/61-logistics-rail.js`. Every downstream logistics owner is textually and behaviourally unchanged.

**Read-only by construction.** `applied:false`, `tracedFriction` consumed by nothing, `friction` unmoved —
and `campaignKind` exists on disk only inside `src/116-conquest-state.js`'s detached factory/view, so no live
campaign, save, load or menu path can reach the new branch at all. The A/B legs confirm rather than establish
it: the standing 24-scenario × 8-seed direction battery is byte-identical
(`18f609d07b1190904ec0c11e4ca64675` at `0829d8d` and at this head) and `probe-full-campaign` is byte-identical
(`a38185fd371a7f181250eff3a6cbf76a`), both 0 diffs, both `ok=true`, 0 pageerrors.

**The teeth caught a real defect and it was fixed in the code, not the tooth.** An INHERITED ruleset
prototype opened the containment gate; `_lgTraceRuleset` now demands OWN, DATA-VALUED descriptors on `id`
and `version` before consulting the allowlist — the discipline `_trRulesetId` and `_ccsRecord` already hold.
Eleven non-admitted shapes now fail closed with zero accessor invocations. Bind D538-B1 mutated ONE allowlist
value and, without a rebuild, redded ONLY `CONTAINMENT-B` (exit 1, 11/12); restore was byte-identical
(`b33c131501dbd38096d79916ca6c3b3c` pre == post) and the rebuild returned the identical game md5 at 12/12.

**The honest result is recorded, not cured.** The 44 sourced services project into ELEVEN disconnected
components, so the default `US`/`E` route resolves in one rail segment (`CTS-R-02`) while `CS`/`E` does not
resolve at all. That is pinned as a tooth and is the measured reason Slice 5 authors the road layer.
...
Ledger-only. The Contract Relay's hard rule (`COORDINATION.md` §1) requires the lane to carry the
complete acceptance criteria and probe design as committed prose BEFORE any runtime edit, with the
teeth landing in the same commit as the code that greens them — so this commit takes DRIVE, writes the
Slice 1 contract into LANE-022 in full, and moves no `src/`, `data/`, `build/`, manifest, suite,
save-shape or generated byte. Game md5 `859637edd920e386dd9008d5dfc647bb`, srcTree
`003d308af7cbaa8d3512df17a0d8d72b`, frozen base `c9db83fa99230ffb95bdfdfe059f3fb9`, manifest 112, data
65 and suite 140 hold unchanged and are not re-pinned.

The contracted seam is the one D537/`docs/design/conquest-supply-chain-design.md` §3 names: `_lgRoute`
keeps its shipped six-key return and gains ONE guarded tail that, for an exact conquest campaign kind
on the open ruleset only, attaches a derived `trace` field carrying a real path across the read-only
36-territory board. Nothing downstream moves; the trace carries `applied:false` and an unconsumed
`tracedFriction`, so the capped bridge and the whole sim stay byte-identical (D537 ruling 4:
read-only first). Containment fails CLOSED at the ruleset seam and is the declared bind target.

Two baseline A/B artifacts were captured at the clean `0829d8d` head before any edit: the standing
24-scenario × 8-seed direction battery (`tools/sweep-all-battles.mjs`, `ok=true failures=0
pageerrors=0`, artifact md5 `18f609d07b1190904ec0c11e4ca64675`) and `tools/probe-full-campaign.mjs`
(`ok=true steps=4 pageerrors=0`, artifact md5 `a38185fd371a7f181250eff3a6cbf76a`). Both are
timestamp-free and therefore byte-diffable against the post-change run.

Recorded in advance: projecting the 44 sourced services yields ELEVEN disconnected components, so most
traced pairs return `reachable:false` (the default `US`/`E` pair resolves in one rail segment,
`CTS-R-02`; `CS`/`E` does not resolve). That is the honest result of tracing a substrate five research
passes left sparse, and the measured reason Slice 5 authors the road layer.

## 2026-07-23 — D537 Aaron decision session: the whole open-fork ledger is ratified; ARC 7 moves to Mayhem

An interactive decision/planning session, not a build. Aaron ruled every open, incomplete, or
refine-worthy decision in the project — 14 rulings — and nothing in `src/`, `data/`, `build/` or the
generated deliverable moved.

**The finding that reshaped the session.** The seed named E53-v2 as a live keystone, "revert-file-halted
at D269," gating E46, the E47 flips and PM2/PM3, and cited `DECISION-NEEDED-e53-build.md` for its
evidence. Disk disagreed on every point: D272 locked the doctrine and deleted that fork file as
resolved, D273 shipped the build with the full battery green ×2 byte-identical, D275 made the E47
flips live, D277 closed the parity arc, D288 closed E59. Five decisions of drift, and the seed's whole
recommended card sequence rested on it. The HALT rule applied — trust disk, say so — and the session
re-derived a single keystone (ARC 7) instead of two. Four more ledger items proved already-closed the
same way (E55/D265, E58/D273, E50/D353, S47/D373), seven GEA rows proved shipped-with-stale-lines, and
one item the seed did *not* list proved genuinely open (E54 — D275 shipped a narrower valve while the
rally branch is verbatim unchanged). This is now decision-quality floor item (11) on the Joint Strategy
Board: **a session seed is not authority.**

**The ARC 7 ruling.** Three citation-grade research passes — D528 rail, D530 water/sea, D532 roads —
closed at zero established physical windows. Historical transport is blocked by exhausted sources, not
unfinished work. Aaron ratified building the ladder on the **Mayhem** ruleset, which has been public
and separately ruled since D420 and may lawfully author its own content, while Historical stays
evidence-gated. Roads get the two cured rows (`RD-SI06`, `RD-SI13`) as sourced exemplars that teach
road *failure*, with an authored network around them; interchanges are Mayhem-only; slice 1 is a
read-only traced route; and the blockade severs the Confederate sea edge, which is the rail/water/naval
unification Aaron asked for. The seam turned out to be small: `_lgRoute` swaps a static per-battle
friction for a traced path over the 36-territory board, and the already-capped supply bridge below it
is untouched — no new combat channel.

**Also ruled:** E57 second, E46 deliberately behind it (both Piper red-halts died on the clock E57
normalizes), E54 parked on a new trigger after all three of its customers turned out dead, ARC 9 Slice
5 closed `NOT_NEEDED` against D515's own latency measurement so **ARC 9 is complete**, E61 as a
war-weariness meter, ARC 8 continuing at soldier POV in battle receipts, R-7/E33 and D460 scheduled,
and four directives promoted to a NEAR tier — with the Engineering Corps and naval-engine directives
absorbed into the ARC 7 ladder as slices 3 and 4 rather than staying separate backlog items.

**Ships (docs only):** `docs/design/conquest-supply-chain-design.md` NEW · DECISIONS D537 ·
COORDINATION LANE-022 opened at `CONTRACT`/`none` with the Slice 1 contract, plus §6 S-05 closing
S-01..S-04 and adding floor item 11 · REVIEW-QUEUE flips and annotations · V1-CHECKLIST ratified build
order · HANDOFF/mirrors · D535's head archived byte-verbatim. LANE-019 was deliberately left untouched
because `probe-conquest-transport-plan` pins its segment tooth-for-tooth.

## 2026-07-23 — D536 S39 ships: the playstyle picker joins the H0 --h0d-* palette (presentation-only)

The contracted edit landed and gated green: `src/95-playstyle.js` defines the six shared tokens
on its panel wrapper (pinned to the 99-h0 canon), every card/row/header/divider consumes
`var(--h0d-*)`, and all 11 retired invented accents are gone from the module (comments included).
The fourth pre-battle config surface now matches the S25 three. Game `c6f9699a`→`859637ed`,
srcTree `7f2a5e6f`→`003d308a`, five chained re-pins; Bind S39-B1 red/restore exact (only the
node-side canon tooth redded, md5-proven restore); probe-playstyle 15/15, probe-presets 27/27
(sim byte-identity guard), probe-desk 13/13, four plan probes, doc coherence 5/5, zero
pageerrors. Worst replaced text pair 6.23:1, required non-text 12.66:1; the divider's move to
`--h0d-line` (~1.8:1, decorative, 1.4.11-exempt per D245) was a contracted supersession.
REVIEW-QUEUE S39 flips to FIXED-in-D536 and LANE-021 releases to `CONTRACT` / `none`.
...
The exact ordered `RD-E11`, `RD-E15`, `RD-E17`, `RD-E18`, `RD-SI06`, and `RD-SI13`
audit closes at `2 ROAD_GAP_CURED`, `4 ROAD_GAP_UNRESOLVED`, and
`0 ROAD_GAP_DISPUTED`. `RD-SI06` and `RD-SI13` are cured; the four `RD-E*` rows remain unresolved.
The road packet's `NEEDS_MORE_RESEARCH` product verdict remains exact, and zero road services exist.

Every counted source and locator was manually opened and read back. Four used sources append
contiguously as `RDS-66`..`RDS-69`; the original 75,108-byte packet prefix, `RDS-01`..`RDS-65`,
all original registers/sections/verdicts, transport JSON, and every product surface remain exact.
The strongest result comes from resolving existing `RDS-50` to Corse's exact Fourth Division
commander report, Official Records Series I, volume 47, part I, Report No. 32. Pages 337-338 combine
the same 5-11 February unit/route with almost-impassable weather, pioneer labor, miles of corduroy,
wagon passage, and the Binnaker's Bridge pontoon crossing, curing `RD-SI06` alongside Platter.
Pages 340-342 independently place that division at Laurel Hill on 9 March in violent rain, flooded
nearly impassable roads, exhaustion, and a midnight halt, curing `RD-SI13`. Chamberlin's compatible
locality and same-regiment partials are not needed for either floor and create no dispute. The Rolla,
Winchester-Harpers Ferry, Boonville-Arrow Rock, and Marshall-Shreveport evidence remains partial or
noncomposable under the row-specific floors.

New Orleans-origin and `CT-36` remain closed; Boonville, Arrow Rock, and Glasgow remain unassigned;
every Potomac crossing and mode-separation negative remains binding. D532 creates no data, runtime,
state, save, UI, topology, route, movement, service, eligibility, control, condition, receipt,
capacity, or gameplay authority. D525 remains the product head, and all four CTIs remain
`INTERCHANGE_WINDOW_UNADJUDICATED`.

The suite-excluded ten-step guard and Bind G own the result. Bind G removes the load-bearing Corse
support sentence from first cured row `RD-SI06` while retaining its source metadata, making only
`DISPOSITION / SOURCE-ENDPOINT FLOOR` red at 9/10 before byte-exact restore. An independent
identity mutation removes only `RDS-50`'s per-segment readback marker and makes only `SOURCE /
FAMILY / TYPE / LOCATOR RESOLUTION` red at 9/10 before restore. Dedicated proof 10/10, transport 12/12, coherence 5/5,
conquest 8/8, Mayhem 13/13, War Career 24/24, normal build/check, exact archive/allowlist/protected/
artifact audits, and `git diff --check` pass. No browser or full-suite checkpoint is owed because no
protected shared product surface moved. LANE-019 releases to `CONTRACT` / `none`; D514 immediately
reloads the completion ledger without any lane-level `SHIPPED` claim.
