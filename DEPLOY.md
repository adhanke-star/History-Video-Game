...
**Status (updated 2026-07-03, D237 — PUBLISHED):** Aaron approved public web publishing in the D237 decision popup, and it is **DONE**: the repo `origin` → **github.com/adhanke-star/History-Video-Game** is **PUBLIC**, GitHub Pages is enabled (branch `main`, root), and the game is live at **https://adhanke-star.github.io/History-Video-Game/** (`index.html` redirects to `civil_war_generals.html`; plays on iPad/iPhone Safari over HTTPS). Reversal path: `gh repo edit adhanke-star/History-Video-Game --visibility private` (which also disables Pages on the free plan). `gh` CLI installed; helper: `tools/deploy.sh`.

> *(History: the 2026-06-14 scaffold block claimed "nothing published" long after the remote went live — corrected in D236 (ledger S19), and the publish decision itself was made and executed in D237. `PHASE4_HALT.md` lives in `legacy/`.)*
