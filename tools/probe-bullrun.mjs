...
// D141 (H5-i3) deliberately added swamps/towns/forts as distinct render types; H5-i4 wires them through
// the universal terrain cover/move hooks. GOLD_TERRAIN is the full intended sandbox terrain guard, while
// GOLD_COMBAT_TERRAIN separately re-asserts the original hill/woods/wall geometry so legacy cover ground
// cannot silently drift as the new topography gains its own effects.
...
const SETUP = `(() => {
  ...
  try {
    ...
    step('SANDBOX UNREGRESSED + terrain GOLDEN SNAPSHOT (silent-drift guard)', function(){
      ...
      // D141/D142: the legacy combat terrain (hill/woods/wall) must stay byte-identical even though
      // swamp/town/fort topography now exists and, under H5-i4, has universal cover/move effects.
...
/* ==== D230/E37 probe teeth (appended) ==== a standalone run must FAIL LOUDLY: exit nonzero
   ...
   pageerrors. Closes the bare `node tools/probe-bullrun.mjs; echo $?` false-green; the vet suite
...
process.on('beforeExit', (code) => {
  if (code !== 0) return;
  const art = new URL('./shots/probe-bullrun.json', import.meta.url);
  try {
    if (__teethStat(art).mtimeMs < __TEETH_T0 - 2000) { console.error('probe-bullrun: TEETH FAIL - artifact not rewritten this run'); process.exit(1); }
    ...
    if (j.ok === false || pe > 0 || failedSteps > 0) { console.error('probe-bullrun: TEETH FAIL - ok=' + j.ok + ' failedSteps=' + failedSteps + ' pageerrors=' + pe); process.exit(1); }
  } catch (e) { console.error('probe-bullrun: TEETH FAIL - no readable artifact (' + (e && e.message) + ')'); process.exit(1); }
});
