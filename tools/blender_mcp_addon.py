...
def register():
    ...
    # Skip auto-start during a headless --background run (e.g. the scripted install/enable):
    # there is no GUI event loop to pump the socket/timer, and we only want the addon
    # ENABLED + saved to prefs here. In the real GUI launch this is False, so the server
    # starts on load and the MCP bridge connects with zero clicks.
