...
const SETUP = `(() => {
  ...
  try {
    ...
    step('UI D150: Army Register is searchable, filterable, detailed, provenance-marked, and journey-start locked', function(){
      ...
      if(!count || count.textContent.indexOf('1281 of 1281')<0) throw new Error('full registry count missing: '+(count&&count.textContent));   // D384: tracks the 1281-person registry (see the count assertion above)
