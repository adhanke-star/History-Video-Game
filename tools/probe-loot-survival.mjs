...
const SETUP = `(() => {
  ...
  try {
    ...
    step('UI D150: Army Register is searchable, filterable, detailed, provenance-marked, and journey-start locked', function(){
      ...
      if(!count || count.textContent.indexOf('1326 of 1326')<0) throw new Error('full registry count missing: '+(count&&count.textContent));   // D388: tracks the 1326-person registry (see the count assertion above)
