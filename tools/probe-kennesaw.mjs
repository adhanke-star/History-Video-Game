...
const SETUP = `(() => {
  ...
  try {
    ...
    step('DATA present and registered: Kennesaw is single-phase, US attacker / CS defender, fog OFF, after Chattanooga', function() {
      ...
      if (order.indexOf('wilderness') !== order.indexOf('chattanooga') + 1 || order.indexOf('spotsylvania') !== order.indexOf('wilderness') + 1 || order.indexOf('coldHarbor') !== order.indexOf('spotsylvania') + 1 || order.indexOf('petersburgAssaults') !== order.indexOf('coldHarbor') + 1 || order.indexOf('kennesaw') !== order.indexOf('petersburgAssaults') + 1) throw new Error('menu order not Chattanooga -> Wilderness -> Spotsylvania -> Cold Harbor -> Petersburg initial assaults -> Kennesaw: ' + order.join(' -> '));   // D442 reshape: Cold Harbor rank 68.5 inserts between Spotsylvania 68 and the Petersburg initial assaults 69; preserve the full six-battle chronology. D397 reshape: Petersburg initial assaults rank 69 between Spotsylvania 68 and Kennesaw 70. D393: Wilderness rank 67 inserted between Chattanooga 65 and Spotsylvania 68. D391: Spotsylvania sat between Chattanooga and Kennesaw.
