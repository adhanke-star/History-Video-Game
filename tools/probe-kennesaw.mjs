...
const SETUP = `(() => {
  ...
  try {
    ...
    step('DATA present and registered: Kennesaw is single-phase, US attacker / CS defender, fog OFF, after Chattanooga', function() {
      ...
      if (order.indexOf('fortPillow') !== order.indexOf('chattanooga') + 1 || order.indexOf('wilderness') !== order.indexOf('fortPillow') + 1 || order.indexOf('spotsylvania') !== order.indexOf('wilderness') + 1 || order.indexOf('coldHarbor') !== order.indexOf('spotsylvania') + 1 || order.indexOf('petersburgAssaults') !== order.indexOf('coldHarbor') + 1 || order.indexOf('kennesaw') !== order.indexOf('petersburgAssaults') + 1) throw new Error('menu order not Chattanooga -> Fort Pillow -> Wilderness -> Spotsylvania -> Cold Harbor -> Petersburg initial assaults -> Kennesaw: ' + order.join(' -> '));   // D442 reshape: Cold Harbor rank 68.5 inserts between Spotsylvania 68 and the Petersburg initial assaults 69; preserve the full six-battle chronology. D397 reshape: Petersburg initial assaults rank 69 between Spotsylvania 68 and Kennesaw 70. D393: Wilderness rank 67 inserted between Chattanooga 65 and Spotsylvania 68. D391: Spotsylvania sat between Chattanooga and Kennesaw. D466 re-pin (the battery's exact-label red): D463 registered fortPillow at rank 66 between Chattanooga 65 and the Wilderness 67 — this probe's own chain teeth sat outside the banked D463 inventory; the chain now guards the seven-battle chronology.
