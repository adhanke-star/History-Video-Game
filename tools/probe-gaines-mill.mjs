...
const SETUP = `(() => {
  ...
  try {
    ...
    check('ARMY REGISTER PIN: 15 Gaines Mill units produce exactly cmd/nco/pvt rows and total 957', function() {
      var C = makeCampaign('US'); if (typeof _t1InitAll === 'function') _t1InitAll(C);
      var reg = ssPersonRegistry(C), rows = [], groups = {};
      if (reg.people.length !== 957) throw new Error('Army Register total is ' + reg.people.length + ', expected 957');
