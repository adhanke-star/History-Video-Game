...
const SETUP = `(() => {
  ...
  try {
    ...
    check('ARMY REGISTER PIN: 15 Gaines Mill units produce exactly cmd/nco/pvt rows and total 990', function() {
      var C = makeCampaign('US'); if (typeof _t1InitAll === 'function') _t1InitAll(C);
      var reg = ssPersonRegistry(C), rows = [], groups = {};
      if (reg.people.length !== 990) throw new Error('Army Register total is ' + reg.people.length + ', expected 990');   // D364: 957 -> 990 — New Market Heights adds 11 unique units x 3 slots (the whole-registry pin moves whenever a battle ships; the Gaines-specific 45-row/15-unit teeth below are the stable part). Prior: D362 912 -> 957 (this battle's own 15 units)
