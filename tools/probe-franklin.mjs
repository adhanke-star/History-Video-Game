...
const SETUP = `(() => {
  ...
  try {
    ...
    step('DATA present and registered: Franklin is single-phase, CS attacker / US defender, fog OFF, after Cedar Creek', function() {
      ...
      if (!(order.indexOf('atlanta') === order.indexOf('kennesaw') + 1 && order.indexOf('crater') === order.indexOf('atlanta') + 1 && order.indexOf('cedarCreek') === order.indexOf('crater') + 1 && order.indexOf('franklin') === order.indexOf('cedarCreek') + 1)) throw new Error('menu order is not Kennesaw -> Atlanta -> Crater -> Cedar Creek -> Franklin: ' + order.join(' -> '));   // D454 re-pin: D436 registered atlanta at rank 71 between kennesaw and cedarCreek (this probe's tooth was authored pre-D436 and never ran since — VETTING DEFERRED); the chain now guards Kennesaw -> Atlanta -> Cedar Creek -> Franklin. D470 battery root-fix 5 (the battery's exact-label red, the D466 sibling class): D469 registered crater at rank 71.5 between atlanta 71 and cedarCreek 72 and re-pinned this probe's DOM chain variant (line ~241) but MISSED this registry-order conjunct — re-pinned to the shipped five-battle chronology; the tree-wide assertion sweep (registry indexOf + fldScnBtn DOM forms, every pair evaluated against the live order) now finds zero stale sites.
...
const DOM = `(() => {
  ...
  try {
    ...
    step('MENU: Franklin button injects once immediately after Cedar Creek and opens side-choice cards', function() {
      ...
      if (!(ids.indexOf('fldScnBtn_atlanta') === ids.indexOf('fldScnBtn_kennesaw') + 1 && ids.indexOf('fldScnBtn_crater') === ids.indexOf('fldScnBtn_atlanta') + 1 && ids.indexOf('fldScnBtn_cedarCreek') === ids.indexOf('fldScnBtn_crater') + 1 && ids.indexOf('fldScnBtn_franklin') === ids.indexOf('fldScnBtn_cedarCreek') + 1)) throw new Error('menu is not Kennesaw -> Atlanta -> Cedar Creek -> Franklin: ' + ids.join(' -> '));   // D436 reshape: atlanta inserts at rank 71
      var got = null;
