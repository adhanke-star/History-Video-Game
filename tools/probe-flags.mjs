...
(async () => {
  ...
  check('battle meta: every registered historical scenario has explicit metadata (no silent Eastern/ANV fallback)',
    pure.ok && Array.isArray(P.metaCoverage) && P.metaCoverage.length === 25 && P.metaCoverage.every(x => x.explicit),   // D391: 21 -> 22 — Spotsylvania ships explicit E/true/anv metadata. D393: 22 -> 23 — Wilderness ships explicit E/true/anv metadata. D397: 23 -> 24 — Petersburg initial assaults ships explicit E/true/anv metadata. D436: 24 -> 25 — Atlanta ships explicit W/false/hardee metadata.
