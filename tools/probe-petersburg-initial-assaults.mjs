...
const SETUP = `(() => {
  ...
  try{
    ...
    check('REGISTRY + MENU: Petersburg initial assaults is rank 69 between Spotsylvania and Kennesaw in the 24-scenario registry',function(){
      var reg=fldScenarioRegistry(),order=fldScenarioMenuOrder(reg);
      if(Object.keys(reg).length!==24||reg.petersburgAssaults!==DATA)throw new Error('registry identity/count wrong');
      if(fldScenarioMenuRank('petersburgAssaults')!==69)throw new Error('menu rank wrong: '+fldScenarioMenuRank('petersburgAssaults'));
      if(order.indexOf('petersburgAssaults')!==order.indexOf('spotsylvania')+1||order.indexOf('kennesaw')!==order.indexOf('petersburgAssaults')+1||order.indexOf('spotsylvania')!==order.indexOf('wilderness')+1)throw new Error('menu chronology wrong: '+order.join(' -> '));
      return {count:Object.keys(reg).length,rank:69,after:'spotsylvania',before:'kennesaw'};
    });
    ...
    check('ARMY REGISTER PIN: canonical registry identity plus 26 Petersburg unit trios produce current total 1512',function(){
      ...
      if(reg.people.length!==1512)throw new Error('Army Register total '+reg.people.length+' expected 1512');   // D397: 1434 -> 1512 — Petersburg initial assaults adds 26 unique side-unit ids x 3 slots. D393: 1380 -> 1434 — Wilderness adds 18 unique side-unit ids x 3 slots.
