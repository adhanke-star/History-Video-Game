...
function staticPreflight() {
  ...
  check("after-action file frozen", md5(join(ROOT, "src", "82-after-action.js")) === "590f8a784609305aa253c08a0fb83d95", md5(join(ROOT, "src", "82-after-action.js")));   // D443 (AD-6 re-pin): e2a4739946b20b1a725a08d55b4825f6 -> cecf9d405c0b335bea3f5ce81347c139 — D434 (GEA-02) legitimately added the authorized Copy/Download export bar to src/82; the war-career surface itself did not move it. D453 (AD-0 battery re-pin): cecf9d405c0b335bea3f5ce81347c139 -> 590f8a784609305aa253c08a0fb83d95 — D451 (GEA-14) legitimately added the guarded spPacketButtonHtml seam to the src/82 export bar (VETTING DEFERRED, so this pin's sweep was owed to the final audit); the war-career surface itself did not move it
  check("Auto file frozen", md5(join(ROOT, "src", "87-auto-resolve.js")) === "9396ff63e2feb299a90c2640c29ace51", md5(join(ROOT, "src", "87-auto-resolve.js")));   // D453 (AD-0 battery re-pin): 4f0bd0970ef96c09b62ea44694387f80 -> 9396ff63e2feb299a90c2640c29ace51 — D448 (GEA-09 phase 1) legitimately tagged the src/87 outcome cue critical through the guarded fldAudioBusPlay funnel (legacy playSfx fallback intact); the war-career surface itself did not move it
  check("T2 file frozen", md5(join(ROOT, "src", "tactical", "T2-campaign-link.js")) === "57e82cd4b9873d8b56ecba51f05a4111", md5(join(ROOT, "src", "tactical", "T2-campaign-link.js")));   // D425: feef8a3c -> 25b7c205 (the recorded D420/LANE-007 Slice C consequence-metadata carry in fldCampaignComputeOutcome/ApplyOutcome; this probe had not rerun since D413). D453 (AD-0 battery re-pin): 25b7c20563be53cadd7ee1ba98a62a3b -> 57e82cd4b9873d8b56ecba51f05a4111 — D448 tagged the T2 campaign-outcome cue critical AND D452 (Mayhem Slice F) added the gated skirmish Ruleset segment + the exact-copy opts.ruleset carry; the war-career surface itself did not move it
...
const SETUP = `(() => {
  ...
  try {
    ...
    step('D411 REACHABILITY + SOURCE-BOUNDED SERVICE', function() {
      ...
      var people = ssPersonRegistry(C).people;
      if (people.length !== 1617) throw new Error('Army Register moved: ' + people.length);   // D436: 1512 -> 1566 — Atlanta adds 18 unique side-unit ids x 3 slots. D442: 1566 -> 1614 — Cold Harbor adds 16 unique side-unit ids x 3 slots D460: 1614 -> 1617 — Elkhorn Cherokee OOB (D455 SS3 row 7): Watie's 2nd CMR adds 1 unique side-unit id x 3 slots.
      ...
      return { runId:C.runId, rolls:expectedRolls, merit:28, reputation:21, promotions:6, rank:'Brig. Gen.',
        role:role.id, latestScenario:'nashville', latestBattleYear:1864, register:1617, boundsCarriers:1,   // D460: 1614 -> 1617 (Elkhorn Cherokee +1x3)
...
async function main() {
  ...
  let result = {
    ...
    ok: false,
    suite: { expected: 137, actual: 0, index: 38 },   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff). D446: 135 -> 136 (concept-links)
    static: staticResult,
    steps: [],
    pageerrors: [],
    realErrors: [],
  ...
  try {
    ...
    result = Object.assign(result, runtime, {
      schema: "cw_probe_war_career_v1",
      generatedAt: new Date().toISOString(),
      suite: { expected: 137, actual: list.length, index: 38 },   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff). D446: 135 -> 136 (concept-links)
      static: staticResult,
      pageerrors,
      realErrors,
    ...
    result.ok = !!runtime.ok && staticResult.ok && !failed.length && !pageerrors.length && !realErrors.length && list.length === 137;   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff). D446: 135 -> 136 (concept-links)
  } catch (error) {
    result.ok = false;
