...
const SETUP = `(() => {
  ...
  try {
    ...
    step('D411 REACHABILITY + SOURCE-BOUNDED SERVICE', function() {
      ...
      var people = ssPersonRegistry(C).people;
      if (people.length !== 1614) throw new Error('Army Register moved: ' + people.length);   // D436: 1512 -> 1566 — Atlanta adds 18 unique side-unit ids x 3 slots. D442: 1566 -> 1614 — Cold Harbor adds 16 unique side-unit ids x 3 slots
      ...
      return { runId:C.runId, rolls:expectedRolls, merit:28, reputation:21, promotions:6, rank:'Brig. Gen.',
        role:role.id, latestScenario:'nashville', latestBattleYear:1864, register:1614, boundsCarriers:1,
