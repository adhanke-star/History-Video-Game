...
function staticPreflight() {
  ...
  check("suite is 135", rows.length === 135, rows.length);   // D425: 130 -> 131 (D418 enrolled the Mayhem row; this probe had not rerun since D413). D443 (AD-6): 131 -> 133 — the D436 atlanta and D442 cold harbor rows appended at the END; the D436/D442 sweeps moved only this probe's 1566/1614 pins and missed this count tooth (recorded honestly). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff, both at the END).
...
async function main() {
  ...
  let result = {
    ...
    ok: false,
    suite: { expected: 135, actual: 0, index: 38 },   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff)
    static: staticResult,
    steps: [],
    pageerrors: [],
    realErrors: [],
  ...
  try {
    ...
    result = Object.assign(result, runtime, {
      schema: "cw_probe_war_career_v1",
      generatedAt: new Date().toISOString(),
      suite: { expected: 135, actual: list.length, index: 38 },   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff)
      static: staticResult,
      pageerrors,
      realErrors,
    ...
    result.ok = !!runtime.ok && staticResult.ok && !failed.length && !pageerrors.length && !realErrors.length && list.length === 135;   // D425: 130 -> 131 (D418 Mayhem row). D443: 131 -> 133 (D436 atlanta + D442 cold harbor rows at the END). D444: 133 -> 134 (learn-battle). D445: 134 -> 135 (chief-of-staff)
  } catch (error) {
    result.ok = false;
