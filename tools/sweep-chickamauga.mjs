...
// tools/sweep-chickamauga.mjs - headless A/B balance sweep for the Chickamauga 3-phase epic.
// Runs N seeds AI-vs-AI and reports, per phase, who held + US/CS casualties (and the
// attacker-pays ratio), plus the aggregate winner/score. Target (D74, no per-battle fudge):
//   P1 The Woods       - contested, leaning CS (the woods deny either side a decision)
//   P2 Breakthrough    - decisive CS win (the Wood gap; CS rolls up the right)
//   P3 The Rock        - US holds (Thomas's defensive success), and the CS ATTACKER pays
//                        more than the US defender (the failed-assault lesson)
//   aggregate          - a clear Confederate tactical victory in the majority
...
const RUN = `(function(seeds) {
  function runChickamauga(seed) {
    __FIELD._officersOff = false; __FIELD._logisticsOff = false; __FIELD._armsOff = false;
    delete G.settings.tacticalFog;
    fldLaunchSandbox({ renderer: 'none', scenario: 'chickamauga', autoBoth: true, seed: seed });
    __FIELD.phase = 'battle'; __FIELD.paused = false;
    var n = 0;
    while (__FIELD.phase === 'battle' && n < 120000) { fldSimStep(0.05); n++; }
    return {
      seed: seed, w: __FIELD.winner, by: __FIELD.winBy, score: __FIELD.phaseScore, cas: __FIELD.battleCas, steps: n,
  ...
  try {
    ...
    var runs = seeds.map(runChickamauga);
...
(async () => {
  ...
  console.log('CHICKAMAUGA SWEEP — ' + N + ' seeds [' + SEEDS.join(',') + ']');
  console.log('aggregate winner: US ' + (agg.US || 0) + '/' + N + ' · CS ' + (agg.CS || 0) + '/' + N + ' · draw ' + (agg.draw || 0) + '/' + N + '  (target: CS majority)');
  for (let p = 0; p < nPhases; p++) {
    const us = Math.round(phaseCas[p].us / N), cs = Math.round(phaseCas[p].cs / N);
    // attacker is CS in all Chickamauga phases -> CS:US ratio >1 means the ATTACKER (CS) bleeds more
    const csPaysRatio = us > 0 ? (cs / us).toFixed(2) : 'inf';
    console.log('  P' + p + ' ' + (out.runs[0].log[p].name).slice(0, 30).padEnd(30) +
      ' | held: US ' + (phaseHold[p].US || 0) + ' CS ' + (phaseHold[p].CS || 0) + ' draw ' + (phaseHold[p].draw || 0) +
      ' | meanCas US ' + us + ' / CS ' + cs + ' | CS:US ' + csPaysRatio);
  }
  let aUS = 0, aCS = 0; out.runs.forEach(r => { aUS += r.cas.US; aCS += r.cas.CS; });
  console.log('  aggregate meanCas: US ' + Math.round(aUS / N) + ' / CS ' + Math.round(aCS / N) + ' (total ' + Math.round((aUS + aCS) / N) + '/battle)');
  console.log('  samples:', out.runs.slice(0, 4).map(r => r.log.map(e => e.w).join('') + '=' + r.w + '(US' + r.score.US + '/CS' + r.score.CS + ')').join('  '));
