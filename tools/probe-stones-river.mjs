...
// Beatty rank traps, the D74 wall, and the current Army Register 1170 pin.
...
const SETUP = `(() => {
  ...
  try {
    ...
    check('ARMY REGISTER PIN: 26 Stones River units produce exact cmd/nco/pvt trios and current total 1170', function() {
      ...
      if (reg.people.length !== 1170) throw new Error('Army Register total is ' + reg.people.length + ', expected 1170');   // D378: 1125 -> 1170 — Cross Keys / Port Republic adds 15 unique units x 3 slots; Stones River's own 78-row/26-unit teeth below remain stable.
