...
const SETUP = `(() => {
  ...
  try {
    ...
    check('ARMY REGISTER PIN: 26 Stones River units produce exact cmd/nco/pvt trios and current total 1200', function() {
      ...
      if (reg.people.length !== 1200) throw new Error('Army Register total is ' + reg.people.length + ', expected 1200');   // D380: 1170 -> 1200 — Five Forks adds 10 unique side-unit ids x 3 slots; Stones River's own 78-row/26-unit teeth below remain stable.
