...
const SETUP = `(() => {
  ...
  try {
    ...
    check('REGISTRY + MENU: Spotsylvania remains rank 68 in the 29-scenario registry after Wilderness, before Cold Harbor', function(){
      var reg=fldScenarioRegistry(),order=fldScenarioMenuOrder(reg);
      if(Object.keys(reg).length!==29||reg.spotsylvania!==DATA) throw new Error('registry identity/count wrong');   // D397: 23 -> 24 — Petersburg initial assaults registers at rank 69. D393: 22 -> 23 — the Wilderness registered. D442: 24 -> 26 — Atlanta (D436, whose sweep missed this count pin — recorded honestly) and Cold Harbor (rank 68.5) both register. D463: 26 -> 27 — Fort Pillow registers at rank 66 between Chattanooga and the Wilderness (LANE-013 P4, the D455 SS3 row 6 unlock). D469: 27 -> 28 — The Crater registers at rank 71.5 between Atlanta and Cedar Creek (LANE-015, the D464 spec). D470: 28 -> 29 — Olustee registers at rank 65.5 between Chattanooga and Fort Pillow (LANE-016, the D465 spec).
      if(fldScenarioMenuRank('spotsylvania')!==68) throw new Error('menu rank wrong: '+fldScenarioMenuRank('spotsylvania'));
      if(order.indexOf('olustee')!==order.indexOf('chattanooga')+1||order.indexOf('fortPillow')!==order.indexOf('olustee')+1||order.indexOf('wilderness')!==order.indexOf('fortPillow')+1||order.indexOf('spotsylvania')!==order.indexOf('wilderness')+1||order.indexOf('coldHarbor')!==order.indexOf('spotsylvania')+1||order.indexOf('petersburgAssaults')!==order.indexOf('coldHarbor')+1||order.indexOf('kennesaw')!==order.indexOf('petersburgAssaults')+1) throw new Error('menu chronology wrong: '+order.join(' -> '));   // D470 reshape: Olustee (rank 65.5) inserts between Chattanooga and Fort Pillow (LANE-016). D463 reshape: Fort Pillow (rank 66) inserts between Chattanooga and the Wilderness; the seven-battle chronology stays guarded. D442 reshape: Cold Harbor (the documented 68.5) inserts between Spotsylvania and the Petersburg initial assaults; the six-battle chronology stays guarded. D397 reshape: Petersburg initial assaults (rank 69) between Spotsylvania and Kennesaw. D393 reshape: Wilderness between Chattanooga and Spotsylvania; D391 guarded the prior three-battle chronology.
    ...
    check('ARMY REGISTER PIN: canonical registry identity plus 18 Spotsylvania unit trios produce current total 1710', function(){
      ...
      if(reg.people.length!==1710)throw new Error('Army Register total '+reg.people.length+' expected 1710');   // D391: 1326 -> 1380 — Spotsylvania adds 18 unique side-unit ids x 3 slots. D393: 1380 -> 1434 — Wilderness adds 18 unique side-unit ids x 3 slots. D397: 1434 -> 1512 — Petersburg initial assaults adds 26 unique side-unit ids x 3 slots. D436: 1512 -> 1566 — Atlanta adds 18 unique side-unit ids x 3 slots. D442: 1566 -> 1614 — Cold Harbor adds 16 unique side-unit ids x 3 slots. D460: 1614 -> 1617 — Elkhorn Cherokee OOB (D455 SS3 row 7): Watie's 2nd CMR adds 1 unique side-unit id x 3 slots. D463: 1617 -> 1632 — Fort Pillow adds 5 unique side-unit ids x 3 slots (LANE-013 P4, the D455 SS3 row 6 unlock). D469: 1632 -> 1671 — The Crater adds 13 unique side-unit ids x 3 slots (LANE-015, the D464 spec). D470: 1671 -> 1710 — Olustee adds 13 unique side-unit ids x 3 slots (LANE-016, the D465 spec).
...
async function main() {
  ...
  try {
    ...
    result.steps.push({name:"CLASSIC + RAIL COLLISION: the frozen lowercase Classic spotsylvania row and the pre-existing strategic rail route remain exact, separate layers (the shiloh/franklin same-name convention)",ok:!!classicOk,v:{classicRows,classicExact,route,craterData}});
