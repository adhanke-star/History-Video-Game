...
const SETUP = `(() => {
  ...
  try {
    ...
    check('ARMY REGISTER PIN: 11 New Market Heights units produce exact cmd/nco/pvt trios and current total 1326', function() {
      ...
      if (reg.people.length !== 1326) throw new Error('Army Register total is ' + reg.people.length + ', expected 1326');   // D388: 1281 -> 1326 — Elkhorn Tavern adds 15 unique side-unit ids x 3 slots. D384: 1200 -> 1281 — Fort Donelson adds 27 units x 3 slots. D380: 1170 -> 1200 — Five Forks adds 10 unique units x 3 slots; the NMH-specific 33-row/11-unit teeth below remain stable.
