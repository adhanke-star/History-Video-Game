...
step("BASELINES: the current D400 whole-registry boundary the arc must not move still holds", () => {
  ...
  if (scenarioCount !== 29) throw new Error("scenario registry must stay 29, counted " + scenarioCount);   // D469: 27 -> 28 — the crater registry line (LANE-015). D470: 28 -> 29 — the olustee registry line (LANE-016).   // D391: 21 -> 22 — Spotsylvania registered as the twenty-second scenario. D393: 22 -> 23 — Wilderness registers after Chattanooga. D397: 23 -> 24 — Petersburg initial assaults registers at rank 69. D436: 24 -> 25 — Atlanta registers at rank 71. D442: 25 -> 26 — Cold Harbor registers at the documented 68.5. D463: 26 -> 27 — Fort Pillow registers at rank 66 (LANE-013 P4, the D455 SS3 row 6 unlock).
  const loot = read(LOOT);
  if (!/people\.length\s*!==\s*1710/.test(loot) || !/1710 of 1710/.test(loot)) throw new Error("Army Register exact D470 whole-registry pin 1710 missing from probe-loot-survival");   // D391: 1326 -> 1380 — Spotsylvania adds 18 unique side-unit ids x 3 slots. D393: 1380 -> 1434 — Wilderness adds 18 unique side-unit ids x 3 slots. D397: 1434 -> 1512 — Petersburg initial assaults adds 26 unique side-unit ids x 3 slots. D436: 1512 -> 1566 — Atlanta adds 18 unique side-unit ids x 3 slots. D442: 1566 -> 1614 — Cold Harbor adds 16 unique side-unit ids x 3 slots. D460: 1614 -> 1617 — Elkhorn Cherokee (D455 SS3 row 7; the D460 sweep MISSED this never-run pin site — it was red on clean 9ad44ea, the D453/D454 stale-pin class, root-fixed here honestly). D463: 1617 -> 1632 — Fort Pillow adds 5 unique side-unit ids x 3 slots (LANE-013 P4).
  ...
  if (suite.length !== 142) throw new Error("vet suite must stay 142, counted " + suite.length);   //D469: 138 -> 139 — the crater row appends at the END (LANE-015).   // D391: 126 -> 127 — probe-spotsylvania enrolled. D393: 127 -> 128 — probe-wilderness enrolls with the twenty-third battle. D397: 128 -> 129 — probe-petersburg-initial-assaults enrolls with the twenty-fourth battle. D400: 129 -> 130 — probe-war-career enrolls with Slice A. AD-7 re-pin (D443): 130 -> 131 — probe-mayhem-mode enrolls (D418); 131 -> 132 — probe-atlanta (D436); 132 -> 133 — probe-cold-harbor (D442), each appended at the END. D444: 133 -> 134 — probe-learn-battle (GEA-07). D445: 134 -> 135 — probe-chief-of-staff (GEA-08). D446: 135 -> 136 — probe-concept-links (GEA-10), each at the END. D447: 136 -> 137 — probe-memory-chain (GEA-12). D463: 137 -> 138 — probe-fort-pillow (LANE-013 P4), each appended at the END. · D538: 140 -> 142 (two LANE-022 Slice 1 conquest-supply rows appended at the END; war career 38 / mayhem 57 hold)
