...
**Live head as of this edit (2026-06-26): Phase H's H5 UG:G-fidelity modern-engine push is complete through D142. The modern engine now has UG:G-style movement/orders (D139), visual fidelity (D140), terrain readability (D141), and fortifications depth (D142): staged works, facing-aware cover, prepared abatis, visible 2D/3D earthworks + HUD, player charge-impetus commit lock, and universal swamp/town/fort movement/cover hooks. Verified gate: build GATE OK, focused H5-i4 probes, presets 26/26, phased A/B 20/0-diff, full no-regression rerun after the final fix, Classic diag `nonBlank:346`, 0 pageerrors, and `git diff --check` clean. Blender is closed and does NOT need reopening for current code/browser/Three.js work; reopen only for an explicit future Phase-H asset job. Next priority = H2 reenactment-footage decision or Phase I/J polish; Q5 Chattanooga + Q6 USCT playable battles remain LAST.** The block below is self-updating — it points at `HANDOFF.md`'s top block + `git log` for live state, so it stays correct as milestones ship.
