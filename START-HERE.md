...
> **ultracode** — Autonomous ALL-NIGHT build run for "The Civil War" (personal project at `~/Desktop/Video Game`; NOT MJI). Full autonomy, $0/single-file, build bar 200/100. You have ZERO context — everything lives on disk; load it, then execute the backlog continuously without me. A 23-round charter session pre-cleared every fork, so **never stall** — decide-&-log and keep building until morning.
>
> **STEP 1 — LOAD CONTEXT, in order:** (1) `START-HERE.md` — the master index. (2) **`AUTONOMOUS-RUN.md` — THE operating manual. Read §8 THE OVERNIGHT CHARTER FIRST (68 Aaron-locked decisions; it governs everything), then §2 live state (note: A1 Cannon Corps is SCAFFOLDED but unbuilt — finish it first), §3 the build loop, §5 guardrails, §7 the charter-integrated priority order.** (3) `DECISIONS.md` — every resolved fork incl. **D30–D41 (the charter)**; don't relitigate, append new ones. (4) `GRAND-STRATEGY-PLAN.md` — the original design LAW (63 decisions + §27 balance; the charter §8 supersedes where they differ). (5) `src/00-manifest.json` + every `src/*.js`, `tools/build.mjs` + `tools/probe-*.mjs`. (6) run `git log --oneline -15`.
>
> **STEP 2 — EXECUTE, autonomously, in this order (full detail = `AUTONOMOUS-RUN.md` §7 priority block):** **(0) FINISH A1 Cannon Corps** — it's scaffolded (`data/artillery.json` + `src/56-artillery.js` + wiring + `tools/probe-cannon.mjs`, `build.mjs --check` already GATE OK): run `node tools/build.mjs` → `probe-cannon` + the full no-regression suite (READ the JSON) → adversarial bug-hunt → commit. **(0b) SET UP GITHUB** — create a PRIVATE repo **`History-Video-Game` under the `adhanke-star` account** (`gh repo create adhanke-star/History-Video-Game --private --source=. --remote=origin` then `git push -u origin main`), and **push after EVERY milestone all night.** Then the battle layer (A2 Engineering Corps → A3 terrain cover → A4 Armory→field → A5 targeting → A6 battle-day conditioning **+ the new bridge AUTO-RESOLVE so the war is playable end-to-end**), then the charter-expanded strategic arc S2→S5 (full cabinet, advisor UX, 3-layer morale, press, command/generals, emancipation, civil liberties, events; codex+glossary+tutorial+WCAG-AA+presets; victory+graded report+Reconstruction coda), then the content systems + theaters (Eastern→Western→…), then tactical P0–P5. **Per item:** plan → (research workflow only if content is genuinely missing) → new `src/NN-*.js` that EXTENDS (or, where §8.2 authorizes, OVERRIDES the frozen engine via a NEW module — never edit `build/base.html`; gate with `diag-classic`) → `node tools/build.mjs` (`GATE OK`) → `tools/probe-<item>.mjs` you READ (start `python3 -m http.server 8765` yourself; `2>/dev/null`; read `tools/shots/*.json`) + full no-regression → **adversarial bug-hunt workflow** → fix to green/0 pageerrors → auto-tune balance + log → commit + **push** + append `RUN-LOG.md`/`DECISIONS.md` → update `AUTONOMOUS-RUN.md` §2 + `HANDOFF.md` → `ScheduleWakeup` (`<<autonomous-loop-dynamic>>`) → next. **At session end write a root `WAKE-UP.md`** (what shipped / what's playable / a demo click-through / open Qs / flags).
>
> **CHARTER PARAMETERS (locked, §8 — don't re-ask):** halt ONLY for irreversible/destructive or money/external-account actions (GitHub push is pre-authorized), else decide-&-log & never stop · **frozen-engine overrides AUTHORIZED** (new modules only, gated by `diag-classic`, revert on regression) · `_SAVE_VER` bumps OK w/ idempotent lazy migration · **unlimited compute** (ultracode workflows aggressively: research fan-outs, parallel builds, adversarial bug-hunt before EVERY commit) · **auto-tune balance + log every number** · art = code-only (CSS/SVG/procedural; NO Blender/3D) · content = ≥2-source Verified else Inferred, anti-Lost-Cause, period-but-tight voice · **FULL WCAG 2.2 AA** + the a11y modes · **MAXIMUM SYSTEMS SHIPPED** (breadth) · NEVER regress Classic (`diag-classic` every change) · single-file may grow to several MB · push every milestone to `adhanke-star/History-Video-Game`. **Tooling note:** the task-output tmpfs throws a spurious "0MB free" ENOSPC — run probes foreground, read `tools/shots/*.json`, verify commits via `.git/logs/HEAD`, clear `/private/tmp/claude-501/*/tasks/*.output` if Bash blocks.
>
> **CONFIRM in 3 lines:** (a) current state + latest commit + that A1 is scaffolded, (b) your plan to finish A1 + set up the GitHub repo, (c) the next 3 backlog items. Then build continuously all night. Do not stop to check in.
