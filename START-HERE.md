...
**Live head as of this edit (2026-06-15): Phase A ✅ (D62) + B-1 attacker AI ✅ (D64) + B-2 officers/command ✅ (D65) + B-3 in-battle logistics ✅ (D66) shipped + pushed; first unchecked = Phase B-4 (distinct arm roles — artillery canister/long-range, cavalry scout/flank/screen/raid; design drafted in `.tmp/B4-arm-roles-design.md`).** The block below is self-updating — it points at `HANDOFF.md`'s top block + `git log` for live state, so it stays correct as milestones ship.
...
>
> **STEP 1 — LOAD CONTEXT (in order, READ them):** (1) **`START-HERE.md`** (this map). (2) **`V1-CHECKLIST.md`** — the APPROVED, ordered roadmap; **this is what you build.** (3) **`DECISIONS.md`** — read **D61** (the operating model + the 12 locked forks), **D62** (Phase A connect-the-layers), **D63** (HYBRID subagent model routing), **D64** (Phase B-1 attacker AI), and **D54/D55** FIRST, then skim D30–D60; append new decisions, never relitigate. (4) **`AUTONOMOUS-RUN.md`** §1–§3 + §8. (5) `MODERN-UGG-PLAN.md` + `GRAND-STRATEGY-PLAN.md` §27. (6) `src/00-manifest.json` + the `src/*.js` you'll extend (esp. `src/tactical/T0-field-sandbox.js` [`fldAiUnit`/`fldAiAttacker`/`fldAiDefender`], `T1-bull-run.js`, `T2-campaign-link.js`, `src/85/86/87` the bridge) + `tools/build.mjs` + `tools/probe-*.mjs`. (7) `git log --oneline -15` + `HANDOFF.md`'s top block for live state.
>
> **STEP 2 — THE OPERATING MODEL (DECISIONS D61 — obey it):**
> - **Build the V1 checklist IN ORDER**, starting at the first unchecked item (consult `HANDOFF.md`'s top block + `git log` for the live head — Phase A + B1 are done; **next = Phase B-2 officers/command**, then B3 in-battle logistics · B4 distinct arm roles · B5 difficulty presets · B6 the **CS-player "command either side" mode** — fork #10). Then **C** breadth (Eastern + **Western** + USCT battles + a custom-battle builder) → **D** a **FULL co-equal hex/turn tactical engine** → **E** strategic S3–S5 (alt-history · codex/glossary/tutorial/presets · the dedicated full WCAG-AA pass · victory + graded report + Reconstruction) → **F/G** content systems + theaters → **H** the "make it come to life" graphics + footage + audio pass → **I** loot/survival → **J** polish/meta. (Full detail per item = `V1-CHECKLIST.md`.)
> - **Clear-then-continuous.** The MAJOR forks are already cleared. So **decide-&-log sub-choices within a checklist item and keep moving**, executing the whole cleared scope continuously (loop milestone after milestone). **PAUSE + `PushNotification` + write `~/Desktop/Video Game/DECISION-NEEDED-<topic>.md` ONLY** for a NEW fork that: is uncovered by the checklist, **contradicts a prior shipped decision/teaching** (e.g. the D58/D64 "fog aids the defender" teaching), is irreversible/costs money, or is so ambiguous a wrong guess wastes a whole milestone. On such a pause, keep making progress on OTHER cleared items if any; else stop and notify.
>
