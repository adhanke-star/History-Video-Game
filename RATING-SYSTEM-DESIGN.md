...
| **R-6 — Negative-badge sweep + dev-traits** | Negatives across all 9 battles (provenance-tagged); wire `reputation` evolution into displayed OVR. **[citation-provenance portion DONE — D103:** the catalog-wide sourcing sweep gave every `Verified` record ≥2 real sources + a permanent build-gate (4e) enforcing it; the negative-badge *assignment to rosters* + live dev-traits remain.] | Legendary-roster balance review (≥90 club not >60% CS); reputation level-up reflected in OVR. **+ every Verified record ≥2 sources (gate 4e); probe-ratings citation-invariant step.** |
